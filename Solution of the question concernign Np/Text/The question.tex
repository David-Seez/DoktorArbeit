...
\end{lemma}
	\begin{proof}
	This is an application of the technical $\lambda$-lemma.
 Because, we can define $N=M$ and $U_x$ is our $r$-cell. Then for a smaller $r$-cell we know that $\phi_n(U_x)$ is arbitrary close with the derivative being arbitrary close.
 Now Assume that no such chart of $U_x$ exists, meaning there is a fibre where the map is \textbf{not} monotonly increasing. This means that for any chart there is a $x\in W(\to p)$ and a $v\in U_x$ such that the map 
	...
	On the other hand, we can assume that $T$ is big enough, such that the projection in a Morse chart $h: \phi_T(U_x)\to W(p\to)$ is one to one and even smooth. This induces a chart to $ phi_T(U_x)$ from $W(p \to )$, which again induces a chart to $U_x=\phi_{-T}( phi_T(U_x))$. Now we can take the chart from the (un-)stable manifold theorem which is locally around $p$ a Morse chart $\psi: U\to T_pM=T_p^uM\oplus T_p^sM$. Hence it satisfies that if $\lambda>1$ and $v\in T^u_pM$ we have that $f((v,0))>f(\lambda (v,0))$, or said differently, the map 
	\begin{align*}
		l_v: (1-\epsilon',1+\epsilon') &\to \R \quad \text{for some $\epsilon'>0$} \\
		\lambda \mapsto f\circ (\psi(\lambda v))
	\end{align*} has a negative differential for $v\neq 0$. Lets Assume that its differential is smaller then $\nu$ for all $\lambda \in (1-\epsilon',1+\epsilon')$. 
	\end{proof}
