...
\end{definition}
	...
	\begin{proof}
		...
		\begin{align*}
			...
			\norm{ h^s_v}<\mu_T
		\end{align*}
		We define the map $h_x=h^s_x\oplus \id_{\lambda_p}: \R^{\lambda_p}\to \R^m$. Now $\phi_T(J(U_x))=\Gamma(h^s_x)=\im (h_x)$. Using this we define our global trivialisation:
		\begin{align*}
		...
		\end{align*} Now we can calculate its derivative to conclude the statement: 
		\begin{align*}
				\diff (l_v)(\lambda)
	...
	Let $U$ be a tubular neighbourhood of $W(\to p)çap M^c$ and let $\epsilon$ be big enough, such that $U$ contains $N_p$. Assume that the normal neighbourhood is trivialized via $\Phi$ such that $f\circ  \phi_T$ is fiberwise monotonically decreasing. For each $x$ the intersection $U_x\cap N_p$ is now starshaped, since if $v\in N_p\cap U_x$, then the whole line $\lambda \mapsto \lambda \cdot v$, $\lambda \in [0,1]$ is contained in said intersection. With this we have that $N_p$ is \textbf{homotopic to a tubular neighbourhood} and that should do the trick already...
	\end{proof}
	\begin{comment}
		\begin{lemma}
