...
\noindent	The maps are the following:
	...
	\end{tikzcd}
	\end{center} Bevore we come to this we notice a view basic facts about spheres. Firstly, if we take a sphere $S^k=\{x\in S^{k+1}|\norm{x}=1\}$ and a closed disc $D^{k}\subseteq S^k$ then, by nature of being a disc, there is a map $\tilde{f}: B^k\to D^k$ that reduces to a map $f: S^k\to \frac{D^k}{\partial D^k}$. Now the first question we need to answer is, when in the diagramm
	\begin{center}
		...
		\end{tikzcd}
	\end{center} $f\circ c$ is homotopic to the identity. Or rather induced $\pm\id$ in $K^k(S^k)$. Let $\beta$ be the natural generator of $K^k[S^k]$. Now $(D^k)^c$ (complement of $D^k$) is a contractible subset of $S^k$ and hence $\beta=c^*(\tilde{\beta})$ for a generator $\tilde{\beta}$ of $K^k[S^k\big/ (D^k)^c)] $
\end{proof}
\begin{comment}
