...
\noindent	The maps are the following:
	...
	\end{align*} On the other hand we have an orientation on $S_1\wedge\left(\frac{V_j}{ \partial V_j }\right)$ coming from an orientation of $T^u_pM$
	 (That induces an isomorphis map from $B_{\mathfrak{b}}:\R^{\lambda_{p}}\to T^u_pM$, where $\mathfrak{b}$ dentotes an oriented basis) via the tubular neighbourhood embedding $J:NW(\to p)\cap M^c= W(\to p)\cap M^c \times T^u_pM \to N_p$. 
	...
	\begin{align*}
		...
		x=(t,y)&\mapsto \left(t,q_{\mathfrak{b}}\circ (\restr{J}{J^{-1}(V_j)})^{-1}(y)\right)
	\end{align*}
	...
	\begin{fdiagram}\label{diag: commutation intresting part}
		...
		\end{center}
	\end{fdiagram}
	Bevore we come to this we notice a view basic facts about spheres. Firstly, if we take a sphere $S^k=B^k\big/\partial B^k$ and a closed (smoothly embedded)  disc $D^{k}\subseteq S^k$ then, by nature of being a disc, there is a diffeomorphism $\tilde{f}: B^k\to D^k$ that reduces to a map $f: S^k \to \frac{D^k}{\partial D^k}$. Now the first question we need to answer is, when in the diagramm
	\begin{center}
		...
		\end{tikzcd}
	\end{center} $f\circ c$ is homotopic to the identity. Or rather induced $\pm\id$ in $K^k(S^k)$. 
	
	
	Assume that $S^k=B^k\big/ \partial B^k$ and without restrictions $D^k\subseteq \inti(B^k)$.  Then we have two different charts of $D^k$. One given by the inclusion and one given by $\tilde{f}^{-1}$. Since $D^k$ is compact and the image of the charts are Hausdorf those two maps define a extension to the Alexandroff-compactifications. Hence the transition function between those two charts extends to a homeomorphism of spheres:
	\begin{align*}
