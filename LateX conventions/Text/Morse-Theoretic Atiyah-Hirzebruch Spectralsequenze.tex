...
\begin{proof} Both $\overline{f}^*$ and $g^*$ are isomorphisms
	...
	In the first case we have that the induced maps equal, and hence for $\beta$ being the bott generator of $B^k\big/\partial B^k$ we have that $(\overline{f}^{-1}\circ g)^*=g^*\circ (\overline{f}^{-1})^*=id$ and hence by the lemma bevore, the statement follows. the other case is the same. 
