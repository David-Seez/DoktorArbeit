...
\begin{proof}[Commuting triple connecting homomophisms]
	...
	\begin{align*}
		\left( ~W(\to) \cup N_q~,~S(q\to)~,~S(q\to ) \setminus \bigcup\limits_jV_j ~\right) \to (C,B,A)
	\end{align*} and hence by naturalitiy we have the commuting diagram:
	\begin{center}
	% https://tikzcd.yichuanshen.de/#-FgjBwkmAOCMaCUcna4Mh0MQZDhECQAGZkSN2OiYJjsbisPiYISziAIVCkOT4YhYbdUVguAAqIlcknUmj8pFuOljEXigFyIA
	\begin{tikzcd}
		{K^{\lambda_p-1}\left[S(q\to),S(q\to ) \setminus \bigcup\limits_jV_j \right]} \arrow[r, "\delta_{triple}"] & {K^{\lambda_p}\left[S(q\to ) \setminus \bigcup\limits_jV_j \right]} \\
...
\noindent	The maps are the following:
	...
	\end{equation*} Now we can compose $c_j\circ c$ to get a map between spheres that is also the identity on $S_1$ smashed with a collaps of a contractible space. Therefore this is homotopic an homeomorphism. Furtermore we have an orientation on $S_1\wedge \left(S(q\to) \right)$ coming from an orientation of $T^u_qM$ (let $\mathfrak{b}_q$ denote a ordered basis )via a rescaling of the unstable map $E^u_q:B_1(0)\to W(q\to )\cap M_c$:
	\begin{align*}
	...
	\begin{fdiagram}\label{diag: commutation intresting part}
		...
		\end{center}
	\end{fdiagram}
	Bevore we come to this we notice a view basic facts about spheres. 
	...
	\end{proof}
 To make use of lemma \ref{lem: Subset Discs creates a sphere} we make the following definitins.  
	...
	\begin{center}
		...
		\end{tikzcd}
	\end{center}
	 satisfies the conditions of the statement becore, and hence 
	 \begin{align*}
	 {b^u_p}\circ \tilde{b^u_q}^{-1}\circ \tilde{b^u_q}\circ c\circ c_j\circ (b^u_q)^{-1}=	 	{b^u_p}\circ c\circ c_j\circ (b^u_q)^{-1} \simeq \pm \id
	 \end{align*} where the sign is determined by the comparison of the standart orientation and the orientation induced from $b^u_p\circ \tilde{b^u_q}^{-1}:D^k\to B^k\subseteq S^k$. Let $x=(b^u_q)x_j\in D^k$ we can check if the induced orientation agrees with the standart orientation, by checking 
	 \begin{equation*}
...
\begin{lemma}
	...
	\begin{align*}
		(f^{-1}\circ g)^*: K^k[S^k]\to K^k[S^k]
	\end{align*} is a group isomorphism and hence $\pm \id$. We have the equivalence of $(f^{-1}\circ g)^*=\id$ if and only if for any $x\in A$:
\begin{equation*}
		\det(\diff \restr{f}{A}\big|_x)>0\, .
\end{equation*}
\end{lemma}
\begin{proof}
 Let $x\in D^k$ and $U$ be a neighbourhood of $x$ contained in $\inti(D^k)$. then $\restr{f^{-1}\circ g}{D^k}=\restr{f^{-1}}{D^k}$. Let $y=(f^{-1}\circ g )(x)$ and $V=(f^{-1}\circ g )(U)$. Then the diagramm commutes:
 \begin{center}
 % https://tikzcd.yichuanshen.de/#
 \begin{tikzcd}
 	{K^k[S^k]} \arrow[r, "(f^{-1}\circ g )^*"] \arrow[d, "i"] & {K^k[S^k]} \arrow[d, "i"] \\
 	{K^k[V,\partial V]} \arrow[r, "(f^{-1})^*"]               & {K^k[U,\partial U]}      
 \end{tikzcd}
 \end{center} Now since the image of $f$ is in $D^k$
\end{proof}
\begin{comment}
