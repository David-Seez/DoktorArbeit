...
\begin{definition}
	...
	\begin{remark}
		...
		\begin{equation*}
			\Phi: \bigoplus_{i+j=k}C_i(M,\mathcal{A},\Lambda)\otimes C_j(M,'\mathcal{A}',\Lambda) \to C_k(M\times M',\mathfrak{A}\times \mathfrak{A}',\Lambda)
		\end{equation*}
		Is an isomorphism of groups and furthermore if we denote the boundary morphism in the chaincomplex corresponding to $M$ by $\partial_f$, the one correspondint to $M'$ by $\partial_{f'}$ and the one corresponding to the product $M\times M'$ by $\partial_{f+f'}$  we get an  isomorphism of chain complexes induced by $\Phi$:
		\begin{equation*}
			\left(C_*(M,\mathcal{A},\Lambda)\otimes C_*(M,'\mathcal{A}',\Lambda),(-1)^j\partial_f\otimes 1 +1\otimes \partial_{f'} \right)
			\to 
	...
	\begin{proof}
		...
		Now we want to show that we get a isomorphism of chain complexes and hence need to check if the boundary map commutes with $\Phi$. Let $a$ be a critical point of index $i$ of $f$ and let $a'$ be a critical ppoint of index $j$ of $f'$. On the one hand we can calculate
		\begin{align*}
	...
	\begin{corollary}[The Boundary Morphism]
		Th
	\end{corollary}
