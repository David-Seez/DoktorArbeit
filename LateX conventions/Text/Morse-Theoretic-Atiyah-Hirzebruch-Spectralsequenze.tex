...
\begin{proof}
	...
	Now to show that the derived couple is also exact one continuous the diagram yoga. We do exemplary the equiality of the kernal of $\beta'$ with the image of $\alpha'$:
...
\begin{definition}[Spectral Sequence From an Exact Couple]
	...
	\begin{equation*}
		D_r= \alpha^{r-1}(D),\quad E_r=\ker d_{r-1}\big/ \im d_{r-1},\quad  \alpha_r= \restr{\alpha}{ \alpha^{r-1}(D)} ,\quad \beta_r =[\beta \circ  \alpha^{r-1}], \quad \gamma_r= \gamma, \quad  d_r= \beta_r\circ \gamma_r
	\end{equation*}
...
\begin{corollary}\label{cor: explicit pages of exact sequences arising from exact couples with grading}
	...
	\begin{equation*}
		...
		=\frac{\gamma^{-1}\alpha^rD^{p+r,q-r}}{\beta(\ker \restr{\alpha^r}{D^{p+r-1,q-r}})}
	\end{equation*}
\end{corollary}
...
\end{corollary}
\begin{proof}
	Notice how our original $E^{p,q}$ is supportet in a strip regarding $p$ which is fancy languare for saying that $E^{p,q}=0$ for $p>0$ and for $p\geq m$. These conditinos cary through the whole spectral sequence, as suceeding pages are quotients of preceeding ones respecting the grading. Now the map $d_r$ has bidegree $(-r+2,r-1)$, and hence $d_r=0$ since the domain or codomain is zero , if the $p$ grading is greater then the $p-$length of the supporting strip meaning $|-r+2|>m$ which gives us the claimes $r\geq m-1$.
\end{proof}
