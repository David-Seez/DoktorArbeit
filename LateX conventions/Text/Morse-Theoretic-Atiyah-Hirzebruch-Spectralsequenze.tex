...
\,				.
		...
		\begin{itemize}
			...
			\begin{equation*}
				\restr{\Phi}{{H}_+}(x)=E^u_q\circ B_{\mathfrak{b}^q}^{-1}\circ \pi(x) \quad \text{and}\quad \restr{\Phi}{{H}_-}(x_0,\dots x_{\lambda_q})=(x_0,f_q\circ \rho')\,.
	...
	\end{tikzcd}
		\end{adjustbox}
		\end{fdiagram}
		 Now define $r\coloneq -R$. Then by the lemma above and commutativity we have:
		  \begin{align*}
		  	r\circ f_q\circ c_1 
		  ...
		  $s\circ r$ has. For this we calculate the local degree in some point by checking the siogn of the determinant of the Jacobean.
	\end{itemize} 
