...
\noindent	The maps are the following:
 ...
 \begin{Iprove}
 	...
 	\begin{center}
% https://tikzcd.yichuanshen.de/#N4Igdg9gJgpgziAXAbVABwnAlgFyxMJZARgBpiBdUkANwEMAbAVxiRAB12G6wBzBmAAIAtJ14AnOlAD6wAB7SAVgF8AFADMAlIM6S+AkMtLpMufIRQAmUgAYqtRizacASpzwBbeIIAqshSoAakqGxiAY2HgERNaU1PTMrIggfvJKygDqqgCO7hCaoSaR5kQAzOT2CU7JqQHKwYqF4aZRFiSklpWOSRxcPPxCouwSUv7pGtq6-
\begin{tikzcd}
	...
	0 \arrow[r] & \langle -\grad_{x_j}(f) \rangle \arrow[r] \arrow[u]                        & \langle -\grad_{x_j}(f) \rangle\times \langle b^u_q \rangle \arrow[r, "\pi"] \arrow[u, "\diff E^u_q\big|_{x_j}\circ Q"] & \langle b^u_p \rangle=T^u_pM \arrow[u, "J_{x_j}"] \arrow[r] & 0
\end{tikzcd}
 	\end{center} The map $Q$ identifies $Q: T^u_qM \to T(T^u_qM)$. 
 	The map $\R\times T_{x_j}V_j\to T_{x_j}W(q\to)$ is giben by $t,x\mapsto -t\cdot \grad_{x_j}(f)+x$ where $T_{x_j}V_j$ is viewed as a subset of $T_{x_j}W(q\to)$.  The vertical maps in the bottom are also inclusions. Now the horizontal maps in the bottom row are exact and as maps of oriented vector spaces the are orientation preserving if and only if the counting in the Morse boundary operator counts $\gamma_j$ as positiv. If this is orientation preserving, this means that .. 
 \end{Iprove}
