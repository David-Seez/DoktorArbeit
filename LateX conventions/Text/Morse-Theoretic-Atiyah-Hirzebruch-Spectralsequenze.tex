...
\,				.
		...
		\begin{itemize}
			\item First we define the map $\rho':H_-\to H_+\cap H_-$ to be $B_{\mathfrak{b}^q}^{-1}\circ {E^u_q}^{-1}\circ \rho \circ E^u_q\circ B_{\mathfrak{b}^q}\circ (\cdot 2)\circ \pi$ for all $x$ such that $x_0\neq 1$. Here, the map $\rho$ is given by 
			\begin{align*}
			...
			\end{align*}Furthermore  $(\cdot 2)$ denotes the stretching of $D^k$ (i.e. the image of $\pi$) to be the ball of diameter two. The north pole gets mapped to any element of the equator. (This choice will not matter later)
			\item $\Phi:S^{\lambda_q}\to W(q\to) \cap N_p\cup C_1S(q\to)$ is defined with a case distingtion. We define 
			\begin{equation*}
				\restr{\Phi}{{H}_+}(x)=E^u_q\circ B_{\mathfrak{b}^q}^{-1}\circ \pi(x) \quad \text{and}\quad \restr{\Phi}{{H}_-}(-x_0,\dots x_{\lambda_q})=(x_0,f_q^{-1}\circ \rho')\,.
			\end{equation*}Notice, how this function is continuous and does not depend on the choice taken in the definition of $\rho'$. 
	...
	\end{tikzcd}
		\end{adjustbox}
		\end{fdiagram}
		 Now define $r\coloneq -R$. For example compose $R$ with the reflection $(x_0,\dots x_{\lambda_q})\mapsto (-x_0,\dots,x_{\lambda_q})$. Then $r$ collapses the lower hemissphere. Then by the lemma above and commutativity we have up to homotopy:
		  \begin{align*}
		  	r\circ f_q\circ c_1 
...
So lets expand this definition:
	\begin{align*}
			...
			&=-\pi_-\circ s\circ col \left(-t,f_q\circ \rho'\left(\left(\pi_+^{-1}(x)\right)_{i=1,\dots,\lambda_{q}}\right)\right)\\
			...
			&=-\pi_-\circ f_p \left(-t,f_q\circ \rho'\left(\left(\pi_+^{-1}(x)\right)_{i=1,\dots,\lambda_{q}}\right)\right)\\
			&=-\pi_-\left(-t,{B_{\mathfrak{b}^p}^{-1}\circ \nu_{x_j}\circ J^{-1}}\circ f_q^{-1}\circ \underbrace{B_{\mathfrak{b}^q}^{-1}\circ {E^u_q}^{-1}\circ \rho \circ E^u_q\circ B_{\mathfrak{b}^q}(2\cdot x)}_{=\rho\circ \pi_+^{-1}(x)}\right)\\
			&=-\pi_-\left(-t,{B_{\mathfrak{b}^p}^{-1}\circ \nu_{x_j}\circ J^{-1}}\circ \rho \circ E^u_q\circ B_{\mathfrak{b}^q}(2\cdot x) \right) \\
			&=-{B_{\mathfrak{b}^p}^{-1}\circ \nu_{x_j}\circ J^{-1}}\circ \rho \circ E^u_q\circ B_{\mathfrak{b}^q}(2\cdot x) 
	\end{align*}
