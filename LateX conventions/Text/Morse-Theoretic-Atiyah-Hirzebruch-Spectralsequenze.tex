...
The downwards maps in the Theorem are the induction of the canonical generators. This is done by the following assignement
	...
	\begin{fdiagram}
		\begin{adjustbox}{max width=\textwidth}
% https://tikzcd.yichuanshen.de/#
\begin{tikzcd}
...
\,				.
		...
		\begin{align*}
			...
			(t,x)&\mapsto \left(t,	\left(B_{\mathfrak{b}^p}\circ \nu_{x_j}\circ J^{-1}\right)(x)\right) \, .
		\end{align*}
		We abuse the notation a bit and identify the map $\left(B_{\mathfrak{b}^p}\circ \nu_{x_j}\circ J^{-1}\right)$ with the one defined by quotiening out the boundary. We will leave out the quotiening in the notatino from now on to make the prove easyer to read. The invested reader should check, if the quotiend maps are always well defined!
		...
		\begin{align*}
			...
			x& \mapsto B_{\mathfrak{b}^q}^{-1}\circ {E^u_q}^{-1}\,.
		\end{align*} 
		Hence $f_q=\left(B_{\mathfrak{b}^q}^{-1} \circ {E^u_q}^{-1}\right)^*:K^{-\lambda_p}$
		...
		\item The map $s$ is given by $f_p\circ l_1$.
		\item To find the map $r$ that fits the diagram (to make it commute) is the next goal. First we need to dig into $l_2$: From diagram \ref{diag: Definitino of delta j} we know that $l_2=\beta\circ \text{triv}_{C_1S(q\to)}\circ i_2^*\circ h_2^*$. Hence if the function $r^*§$ makes the diagram commutative we need to ensure, that $f_q^* \circ r^*=l_2$ If we define the function 
		\begin{align*}
			col: W(q\to) \cap N_p \cup C_1  S(q\to) \to S_1 \wedge S(q\to) \,
		\end{align*} 
		to be the collision, of $W(q\to) \cap N_p $ to a point, then
		$i_2^*h_2^*=col^*$. Now while the function  $\text{triv}_{C_1S(q\to)}$ is not topologically induced, its inverse is. lets call the collaps of $C_1S(q\to)$ $c_1$. Then 
		\begin{equation*}
			 \beta \circ f_q^*\circ r^*=l_2\leftrightarrow c_1^*\circ f_q^*\circ r^*=col^*
		\end{equation*}
	\end{itemize} 
