...
\,				.
		...
		\end{proof}
		Now we need to transoprt our situation to fit the lemma. For this we will use the maps defined in the lemma above, and need a bunch of maps again: 
		\begin{itemize}
			\item First we define the map $\rho':H_+\to H_+\cap H_-$ This map is given by 
			\item $\Phi:S^{\lambda_q}\to W)q\to \cap N_p\cup C_1S(q\to)$ is defined with a case distingtion. We define 
			\begin{equation}
				\restr{\Phi}{H_+}(x)=E^u_q\circ B_{\mathfrak{b}^q}^{-1}\circ \pi(x) \quad \text{and}\quad \restr{\Phi}{H_-}(x_0,\dots x_\lambda_q)=(x_0,f_q\circ \rho')
			\end{equation}
			\item $\Psi$
			\item $p_i$ für $i=1,2$
			\item $C_{S(q\to)}$
