...
\begin{definition}[Connected Simple System]
	A \textbf{connected simple system} consists of a cllection $I_0$ of pointed spaces along with a collection $I_m$ of homotopy classes of maps between these spaces such that 
		\begin{enumerate}[(i)]
			\item $\hom(X,Y)\coloneq \{[f]\in [X:Y]:[f]\in I_m\}$ consists of exactly one element for each pair of spaces $(x,y)\in I_0$, where $[X:Y]$ denotes the set of homotopy classes of maps between $X$ and $Y$.
...
\begin{remark}[Critical points as isolated compact invariant subsets] \label{ex: Critical points as isolated compact invariant subsets}
	...
	\begin{equation} 
		C^{k}(M,\mathfrak{A},\tilde{K}^{l}(pt)=\bigoplus_{ \Crit_k(f)} \bigoplus_{q\in \Crit_k(f)} K^k(\mathcal{IP}_q)).
	\end{equation}
	. 
\end{remark}
\begin{remark}
...
\begin{definition}[Regular Index Pairs and a Filtration on $M$] \label{def: Regular index pairs and a filtration on M } \label{def: Regular index pairs and a filtration on M}
	...
	\end{equation} such that $(N_k,N_{j-1})$ is a regular index pair for $W(k,j)$ for all $0\leq j \leq k\leq m$. From this we can induce a filtration of conected simple systems:
	\begin{equation*}
		\emptyset \to \mathcal{IP}_0\to \mathcal{IP}_1 \to \cdots \to \mathcal{IP}_{m-1} \to \mathcal{IP}_m 
	\end{equation*} Where $\mathcal{IP}_i$ denotes the simple system $\mathcal{IP}_{\Crit_i(f)}$ and the map $\mathcal{IP}_i\to \mathcal{IP}_{i+1}$ is induced from the inclusion $i: (N_i,N_{i-1})\to (N_{i+1},N_i)$. \todo{ we get even More! we get a list of such sequiences! maby a spectral sequence?}
\end{definition}
\begin{proof}
