...
The downwards maps in the Theorem are the induction of the canonical generators. This is done by the following assignement
	...
	\begin{fdiagram}
		\begin{adjustbox}{max width=\textwidth}
% https://tikzcd.yichuanshen.de/#
\begin{tikzcd}
	{K^{-\lambda_p}\left[V_j,\partial V_j \right]} \arrow[rr, "\delta^j_{triple}"] \arrow[rd, "l^*"]   &                                                                       & {K^{-\lambda_q}\left[W(\to q)\cap N_p,S(q\to)\right]}                        \\
	& {K^{-\lambda_p+1}\left[ S_1\vee S(q\to)\right]} \arrow[rd] \arrow[ru] &                                                                              \\
	{K^{-\lambda_p+1}\left[\mathbb{S}^{\lambda_p+1} \right]} \arrow[uu, "f_p^*"] \arrow[rr] \arrow[ru] &                                                                       & {K^{-\lambda_p+1}\left[\mathbb{S}^{\lambda_p+1} \right]} \arrow[uu, "f_q^*"]
\end{tikzcd}
		\end{adjustbox}
	\end{fdiagram}
	Here $f_q$ is supposed to induce $\alpha_p$ and $f_q$ induces $\alpha_q$. which in turn are the canonical generators induced by the orientation of the respective unstable tangend spaces. We alredy established, that 
	$f_p$ is given by $\left(\id\times \left(J\circ \nu_{x_j}^{-1}\circ B_{\mathfrak{b}_p}\right)^{-1}\right)$ and $f_q$ is given by 
