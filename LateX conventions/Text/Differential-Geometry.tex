...
\begin{proof} Let $\phi:U \to V$ be a chart of $M$. 
	We proof this by showing it for the local sections $\frac{\partial}{\partial x_i}$ and thereby for any, since those sections form a basis of the space of sections as a $C^{\infty}(M,\R)$ vector space. Now let $g_p\in \mathcal{E}_p(\R)$ and $\phi(p)=x_0$. Then
	\begin{equation*}
		\diff f _p(\partial_i(p))(g_p)= \partial_i((g\circ f)_p)=
		=\restr{\frac{\partial }{\partial x_i}}{x_0}(g\circ f\circ \phi^{-1})=\restr{\frac{\partial }{\partial x}}{p}(g_p) \cdot \restr{\frac{\partial }{\partial x_i}}{p}(f_p)
	\end{equation*}
