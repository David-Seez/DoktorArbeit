...
\begin{longtable}{>{\centering\arraybackslash}p{3cm} p{11cm}}
	...
	% Hier deine Notationen einfügen:
	$i:A\to B$ & If $A\subseteq B$ this will always denote the inclusion \\
	$B_{\mathfrak{b}}:\mathbb{R}^n \to V$ & Isomorphism for the Basis $\mathfrak{b}=(v_i)_i$ of $V$: $(a_1,\dots,a_n)\mapsto \sum a_i v_i$ \\
	$q_\mathfrak{b}:V\to\mathbb{R}^n$ & Coordinate map: $v=\lambda_iv_i\mapsto (\lambda_i)_i$ and also $B_{\mathfrak{b}}^{-1}$ \\
\end{longtable}
\section{To Do Liste}
\begin{itemize}
	\item How exactly is the morse boundary operator gievn? Make use of the map $E^u_q$ and $J$.
	\item tripele boundary operator.
	\item wedge produkt and smash produkt commute...
\end{itemize}
