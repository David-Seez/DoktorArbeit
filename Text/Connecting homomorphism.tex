...
\end{align*} that is the identity on all $\mathring{V_j}$. Furthermore we have the compression maps
\begin{align*}
	c_j: \bigvee_{j=1}^lC_1V_j \big/ \partial C_1 V_j \to C_1V_j \big/ \partial C_1 V_j 
\end{align*}
 Hence by the additivity of $K-$Theorie, the pair
\begin{align*}
	\left( K^p \left[  \frac{ C_1(L_q)}{ \cl \left(C_1(L_q)\setminus \bigcup\limits_{j=1}^l C_1V_j \right) } \right]  ,\left( c_j \circ \Lambda^*\right)_j\right)
\end{align*} satisfies the coproduct universal property.
For all $C_1V_j$ we get an homeomorphismto $S^{k+1}$:
\begin{align*}
