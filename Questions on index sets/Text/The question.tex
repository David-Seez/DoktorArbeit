...
hier habe ich meinen aktuellen Stand zusammengefasst. Am Anfang nenne ich nochmal das Setting und das Lemma \ref{lem: the Main lemma}, das es zu beweisen gilt. Es folgt eine kurze Anmerkung dazu, warum ich das gerne beweisen möchte und wo die Behauptung zu finden ist.

\textbf{Am spannendsten für dich} ist, denke ich, Abschnitt 2: "Where I Stand at the Moment". Dort fasse ich meine bisherigen Ergebnisse zusammen und erkläre meine Probleme. Dabei sind auch querverweise auf den nächsten Abschnitt:

Danach folgt einiges an technischem Material, das die bisherigen Beweise enthält. Das ist vor allem der Vollständigkeit halber im Dokument enthalten.
\section{The Setting}
...
\end{align}
\begin{lemma}\label{lem: the Main lemma}
    We claim that 
    \begin{enumerate}
...
\end{lemma}
\begin{remark}
	For our purpose we don't really need $N_p$ to be a tubular neighbourhood of $W(\to p)\cap M^c$. What we need is a canonical way to identify the connected components $V_i$ of the intersection $N_q\cap N_p=\sqcup_i V_i$ with $W(p\to)$. The tubular neighbourhood tries to argue, that both are fibers in the normal bundle of $W(\to p)\cap M^c$ and since the latter is contractible this can be trivialized.In \cite{MorseTheorySalmbon} in the proof of lemma 3.2 Salamon claims this to be true without a proof (page 119 in the attached source). Similar, in \cite{banyaga2004lectures} Banyaga claims this (also without any argument). A Figure below depicts the situation:
   \begin{center}
...
\end{center}
\end{remark}
\section{Where I Stand at the Moment}    
...
If we have a Morse chart around $p$, where the gradient is euclidean, the statement is at least around $p$ easily done. Compare the proof below (\ref{proof: When the Metric is Euclidean}). (By moving $c$ close to $f(p)$ this should then also show the global statement). But there is now way to assume that the gradient is euclidean and the Morse lemma doesn't care for the gradient (it is a statement for smooth and not Riemannian manifolds) Now in general I was able to manipulate the Morse-Chart, such that the gradient at $p$ is of diagonal form with positiv entries(compare theorem \ref{thm: Simultaneous Diagonalisation of Quadratic Forms}). This gave hope for a Taylor expension of the flow, since the Jacobean of the flow can be expressed via the gradien. With heavy notation this reads: 
\begin{align*}
...
\end{align*} Here the left side is the Jacobian of the flow in a chart in the origin. (The notation is unpacked below) Now there are several \textbf{problems}:
\begin{itemize}
	\item Even if $g^{ij}$ is diagonal and positive definite, it is hard to expand the above expression. whilst maintaining the splitting. compare the current status of that in remark \ref{rem: Linearly Approximate the Flow}.
	\item The proof for the "trivial" case where the metric is euclidean relies on a argument that uses local monotomie after a splitting(compare equation \ref{eq: splitting and monotonie}). This however cannot be derived from a Approximation of the flow. 
	\item The statement of $N_p$ being a tubular neighbourhood is not enough. Implicitly Salaomon also uses, that the $V_i$ which can be denoted as the connected components $f{-1}(c)\cap N_p$ are fibres in the neighbourhood, or canonically homeomorphic (up to homotopy) to fibres. 
\end{itemize}
A little bit more on the second problem. Assume that we have a two dimensional manifold and in our Morse chart the gradient $g^{ij}$ for $i,j\in \{1,2\}$ is given by the matrix
\begin{align*}
g^{12}=g^{21}=0 \quad, g^{22}=1\quad \text{and }\\
g^{11}(x,y)=1+\euler^{\frac{-1}{x^2}} \sin (\frac{1}{x^2})
\end{align*} This comes from a well defined smooth metric and furthermore in the origin, this is the euclidean. However, the monotony is not controled by the linear approximation. (in fact, this pathological example shows that even the Taylor series wouldn't help).
So here I am stuck. This looks like the gradient of the function. I assume that the Morse-Smale condition makes sure that such examples are not possible, but this is something to work on. 
\section{The Technical Parts}
\begin{proof}[Proof of the Tubular neighbourhood: When the Metric is Euclidean]\label{proof: When the Metric is Euclidean}
For now assume that $g_{ik}=\diag(1,\dots 1)$, i.e. that we work with the euklidean metric. Then the gradient is:
\begin{align*}
...
\end{equation}
This however reads: 
\begin{align} \label{eq: splitting and monotonie}
...
\end{equation*} where $R(x^s)$ is a smooth function.
\end{proof}
...
\begin{lemma}
	...
	\end{align*}
	Here, $g^{ij}$ denotes the smooth functions given by inverting the coordinates of the function $x\mapsto (g_{ij}(x))$ that defines the coordinate representation of the gradient in $x$. 
\end{lemma}
...
\begin{lemma}
	Let $\psi$ be a coordinate system arround a critical point $p$. I.e. $\psi: p\in U\to V$ with $\psi(p)=0$ and let $\left( \frac{\partial}{\partial x_1}\big|_x,\dots ,\frac{\partial}{\partial x_m}\big|_x\right)$ be the induced basis of $T_xM$ Then the jacobean of the gradient reads
	\begin{equation}
...
\begin{proof}
	...
	\begin{align*}
		...
		\end{align*}
\end{proof}
\begin{remark}[Linearly Approximate the Flow] \label{rem: Linearly Approximate the Flow}
Now we want to use this to taylor expand the flow at the origin in said chart. 
 We now want to interlinear approximate  $\psi\circ \phi_t \circ \psi^{-1}:U\to U$ which leads to the need of its jacobean. It's not to hard to show that the flow has the Jacobean
\begin{align*}
	q_{\psi}	\frac{\partial}{\partial x}\big|_0 \phi_t(\psi^{-1}(x)	)
	&=\exp\left( \sum_k \left(g^{ki}(0) \frac{\partial ^2 (f\circ \psi^{-1})}{\partial x^j\partial x^k}\right)_{ij} \right) \, .
...
\begin{align*}
	...
	\end{array}
\end{align*} 
For fixed $x^s=a^s$ we inspect the slices and hope they are star-shaped: Assume that $(a^u,a^s)$ satisfies the property:
...
\begin{align*}
	...
	\geq -\epsilon
\end{align*}
, then for any $0\leq \lambda \leq 1$, the point $(\lambda a^u,a^s)$ also satisfies the property: (here i am at the moment....)
The next is just some ideas i tried but did not manage
