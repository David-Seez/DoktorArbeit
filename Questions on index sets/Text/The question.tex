...
\end{enumerate}
\end{lemma}
The gradient is a section into the tangent bundle. Hence it is of the form:
\begin{align*}
    \grad(f):M\to TM
\end{align*}
\begin{lemma}
...
\begin{corollary}
	In a Morse chart the function $f\circ \psi^{-1}-f(p)$ is the two form given by the matrix
	\begin{align*}
		H\coloneq \begin{pmatrix}
			-1_{l\times l}&0 \\
			0&1_{(m-l)\times [m-l]} 
		\end{pmatrix}
	\end{align*}
	Now we have the positive definite matrix $(g^{ij})_{ij}$ given by $g(p)$ with respect to the chart induced basis. The hope is to diagonalize those two matricies simultanious, which is possible since they commute! To see this first we split $R^m$ into the eigenspaces $E_{-1}\oplus E_1$ where 
	\begin{align*}
		E_{-1}\coloneq \{v\in R^m| Hv=-v\} \quad \text{and}\quad 	E_{1}\coloneq \{v\in R^m| Hv=v\}
	\end{align*}
	Now assume $v\in E_\lambda$ with $\lambda\pm1$. Then $H\circ (g^{ij})_{ij} (v)=(g^{ij})_{ij} (Hv)=\lambda (g^{ij})_{ij} (v)$ and hence, $(g^{ij})_{ij} (v)\in E_{\lambda}$ now we need to diagonalize $(g^{ij})_{ij}$ restricted to the two eigenspaces of $H$ which is possible!. \todo{warum und welchen einfluss hat das auf die 2 form durch H?}
\end{corollary}
\begin{lemma}
	Law of inertia Sylvester
\end{lemma}
...
\begin{proof}
	...
	Let $L = [v_1 | \dots | v_m]$ be the matrix whose columns are these $G$-orthonormal eigenvectors. The linear change of coordinates \todo{ab hier selber rechnen!} 
	...
	\end{align*}
	Since $H v_i = \lambda_i G v_i$, we have $L^T H L = \text{diag}(\lambda_1, \dots, \lambda_m)$. The eigenvalues $\lambda_i$ are real and have the same signature as the eigenvalues of $H$ ($l$ negative, $m-l$ positive).\todo{hier ein beweis einfügen}.  By a further scaling of the coordinates $z$, the matrix $L^T H L$ can be brought into the form $\text{diag}(-1, \dots, -1, 1, \dots, 1)$. (by doing this however, the matrix $G$ becomes $G=\diag(\mu_1,\dots \mu_m)$ with $\mu_j>0~\forall j$)
\end{proof}
...
\begin{corollary}
Let $q: R^l\oplus R^{m-l}$, $i_1: R^l\to R^m$ and $i_2 R^{m-l}\to R^m$ be the inclusions and let $q:R^l\oplus R^{m-l}\to \R$ a smooth map such that $q\circ i_1$ is strictly increasing and $q\circ i_2$ strictly decreasing. Let furthermore $g: R^m \times R^m\to R$ be a metric on $R^m$. Then there is a orthonormal basis 

\end{corollary}
\begin{corollary}
	Assume that $\psi$ is a Morse chart and$(l_{ij})\in \mathrm{GS}(l,m)$. Then $f\circ \psi^{-1}\circ L^{-1}$ reads: 
	\begin{align*}
		f\circ \psi^{-1} \circ L \circ L^{-1} (x^1,\dots x^m)=
	\end{align*}
\end{corollary}
\begin{lemma}
