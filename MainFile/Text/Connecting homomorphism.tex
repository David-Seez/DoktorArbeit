...
\begin{lemma}
	ALternatifly, we dont need a homeomorphism $V_j\to T_p^uM$, rater from a subset of $V_j$ (maby a ball contained in $V_j$). then anything outside of $V_j$ can be disregarded. or rather define $V_j$ for a $T$ big enough to be a "ball arround $x_j$ that lifes in our currently defined $V_j$. I Think thats the way!
\end{lemma}
\begin{lemma}[The map $V_j\to D^{k-1}$] First of all we need to make shure, that for $T$ big enough every connected component of $N_p\cap L_q$ contains an element $x_j$ in $W(q\to p)$. Assume that this was not the case. Let $T_i\in \R$ be a sequnce  approaching infinity and $x_i$ be elements in connected components of $N_p\cap L_q$ that do not contain klappt nmicht man ... neue idee muss her!
