...
\begin{definition}[Pullback of a family of vector spaces] \label{def: Pullback of a family of vector spaces}
	If $\pi:E\to Y$ is a family of vector spaces, $X$ a topological space and $f:X\to Y  $ a continuous function. Then we define the induced pullback of the family $f^*{\pi}:f^*(E) \to X$ as follows: 
	$f^*(E)$ is the subspace of $(x,e)\in X\times E$ such that $f(x)=\pi(e)$, and the map $f^*{\pi}$ is the projection to the second factor reduced th said subspace. 
\end{definition}



\begin{corollary} \label{cor: pullback is functoriell}
...
\begin{corollary}\label{cor: sections into hom(E,F) = hom(E,F)}
	We can identify $\Gamma \hom(E,F)$ with $\hom(E,F)$ since for any section $s:X\to \hom(E,F)$ we can define the bundle homomophism $\Tilde{s}:E\to F$ by sending $v\mapsto s(v)$. This is a homomorphism since $\Tilde{\pi}s(v)=\pi(v)$. 
\end{corollary}
...
\end{proof}
\begin{corollary}\label{cor: rank is upper-semi-continuous}
...
\begin{definition}[Trivialisation of a Vector Bundle]\label{def: Trivialisation of a Vector Bundle}
	If $Y$ is a closed subspace of $X$, $E$ is a vector bundle over $X$ and $\alpha:{E}\einsch{Y}\to Y\times V$ is an isomorphism. Then $\alpha$ is a \textbf{trivialisation of $E$ over $Y$}. Furthermore, let $\pi: Y \times V \to V$ denote the projection and define an equivalence relation on ${E}\einsch{Y}$ by 
	\begin{equation*}
...
\end{definition}
\begin{corollary}[Elementary Properties of the Gluing and Clutching Construction]\label{cor: Elementary Properties of the Gluing and Clutching Construction}
...
\end{definition}
\begin{corollary} \label{cor: splitting exact sequenz}
	If  \begin{tikzcd}
...
\begin{definition}[Extreme Kindes of $G$-spaces]
	...
	\end{enumerate}
\end{definition}
\begin{corollary} Suppose that $E$ is a $G-$vector bundle over $X$.
