...
\end{proof}
\begin{corollary} \label{cor: reduced Ktheory is unpointed K-theory}
...
\end{corollary}
\begin{corollary}\label{cor: first iso to reduction of K-group}
...
\begin{proof}
	...
	\end{center} Hence, $j^*\circ i^*$ factors over the trivial Ring and thereby is trivial.
	 ...
	 (Compare remark \ref{rem: Elements in the K-Group are of the form F-n}). Since $i^*\xi=0$ it follows that $i^*([E]-[n])=[{E}v{Y}]-[{n}\einsch{Y}]=0$ and hence $E$ and $n$ are stably equivalent, meaning there exists n $m\in \N$ such that
	\begin{equation*}
...
\end{proof}
\begin{corollary}\label{cor: exactnes of the reduced inclusion sequence}
...
\begin{proof}
	...
	\begin{center}
		...
		\end{tikzcd}
	\end{center}
	The bottom square commutes by definition. In the top square, the map $\tilde{\pi}$ denotes the collapsing map $X\cup C_1Y\cup C_2X\to (X\cup C_1Y)\big/X$. To see the commutativity in the top square notice how $\tilde{\pi}^*\circ s^*=(s\circ \tilde{\pi})^*$ and the maps $m$ and $s\circ \tilde{\pi}$ coincide and hence the top square commutes. we run into a Problem, since the map $l^*$ induces a map $\lambda$ by the “wrong” inclusion (compared to $i^*$):
	\begin{center}
...
\end{proof}
\begin{corollary}\label{cor: retractions induce splitting sequences}
