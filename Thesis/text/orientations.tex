...
Any finite dimensional real vector space $V$ is homeomoprhic to $\R^n$ via the coordinate map $\kappa_v$ that maps each vector to its coordinate representation. With this we can introduce a K-theoretic orientation from an ordered basis via the composition: 
   \begin{center}
\begin{tikzcd}
\tilde{K}^{-n}(S^n) \arrow[r, "(\pi_S^{-1})^*"] & \tilde{K}^{-n}(\overline{\R^n}) \arrow[r, " \kappa_v^*"] & \tilde{K}^{-n}(\overline{V})
\end{tikzcd}
...
\begin{proof} Let $\kappa_v:V \to \R^n$ and $\kappa_w:\R^n \to W$ be two basis maps from ordered basis compatible with the orientations ov $v$ and $W$.
    By definition of the orientation on $V\oplus W$ we know that the direct sum of thhe maps $\kappa_v\oplus \kappa_W$ is an orientation preserving between $V\oplus W$ and the canonically oriented $\R^n\oplus \R^m$. Naturality of $\Xi$ gives us
    \begin{equation*}
       \Xi \circ \kappa_v\oplus \kappa_W=\kappa_v \wedge \kappa_w \circ \Xi\,.
    \end{equation*} Hence: by definition and associativity of the wedge product:
\begin{align*}
    \Xi^{*}\left(\beta_{\mathfrak o}\wedge\beta_{\mathfrak p}\right)
    &=      \Xi^{*}\left(
\kappa_v^*\wedge \kappa_w^*
      \right)\circ \left(
         (\pi^{-1}_s)^{-1} \wedge (\pi^{-1}_s)^{-1}
         \right)\beta_n\boxtimes\beta_m\\
    &=      \left(
\kappa_v^*\oplus \kappa_w^*
      \right)\circ \Xi ^* \circ \left(
         (\pi^{-1}_s)^{-1} \wedge (\pi^{-1}_s)^{-1}
         \right)\beta_n\boxtimes\beta_m\\
         &=
          \left(
        \kappa_v^*\oplus \kappa_w^*
      \right)\circ \left(
         (\pi^{-1}_s)^{-1} \oplus (\pi^{-1}_s)^{-1}
         \right)\circ \Xi^* (\beta_n\boxtimes\beta_m)\\
         &=  \left(
        \kappa_v^*\oplus \kappa_w^*
      \right)\circ \left(
         (\pi^{-1}_s)^{-1} \oplus (\pi^{-1}_s)^{-1}
         \right)\circ \Xi^* (\Upsilon^*_{n,m}(\beta_{n+m}))\\
         &=\left(
        \kappa_v^*\oplus \kappa_w^*
      \right)\circ \left(
         (\pi^{-1}_s)^{-1} \oplus (\pi^{-1}_s)^{-1}
         \right)\circ \Xi^* (\overline{\pi^{n+m}_S}^{-1}\circ \Xi^{-1}\circ \overline{\pi^n_S}\wedge \overline{\pi^m_S})(\beta_{n+m}))\\
         &=\left(
        \kappa_v^*\oplus \kappa_w^*
      \right)\circ \left(
         (\pi^{-1}_s)^{-1} \oplus (\pi^{-1}_s)^{-1}
...
On the other hand:
\begin{equation*}
    \beta_{\mathfrak{o}\oplus \mathfrak{p}}=\kappa_{v\oplus w}^*\circ (\pi^{-1}_S)^*\circ \beta_{n+m}
\end{equation*}
Jetzt bin ich fast fertig hier....
\end{proof}
