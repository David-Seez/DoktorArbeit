...
\end{definition}
\subsection{Noch Durcharbeiten!!!}
\begin{definition}[Orientation preserving charts and maps]
	Let $M$ be an oriented $n$-manifold with orientation given by the nowhere-vanishing
	section $\omega$ of $\det(TM)$, and let $\varphi:U\to \R^n$ be a chart. We call
	$\varphi$ \textbf{orientation preserving} if for every $x\in U$ the elements
	$\omega_x$ and $\omega^\varphi_x$ lie in the same connected component of
	$\det(T_xM)\setminus\{0\}$, where $\omega^\varphi$ denotes the coordinate frame
	section $\partial_1\wedge\dots\wedge\partial_n$ of $\varphi$.

	More generally, let $N$ be an oriented $n$-manifold with orientation $\eta$ and
	let $h:M\to N$ be differentiable at $x$ with $\diff h_x$ invertible. We set
	\begin{equation*}
		\sign\det(\diff h_x)\coloneq
		\begin{cases*}
			+1 & if $\Lambda^n(\diff h_x)(\omega_x)$ and $\eta_{h(x)}$ lie in the same component of $\det(T_{h(x)}N)\setminus\{0\}$,\\
			-1 & otherwise.
		\end{cases*}
	\end{equation*}
\end{definition}
...
\begin{remark}
	The sign is well defined: replacing $\omega$ by $\lambda\omega$ with $\lambda>0$
	rescales $\Lambda^n(\diff h_x)(\omega_x)$ by the positive number $\lambda(x)$, and
	replacing $\eta$ by $\mu\eta$ rescales the comparison element by $\mu(h(x))>0$;
	neither changes the component. Moreover, reversing \emph{both} orientations
	replaces $\Lambda^n(\diff h_x)(\omega_x)$ by its negative and $\eta_{h(x)}$ by
	its negative, so the sign is unchanged. In particular, for $M=N=S^n$ the number
	$\sign\det(\diff h_x)$ does not depend on which of the two orientations of $S^n$
	we choose --- only on orientability.
\end{remark}

\begin{lemma}[Computing the sign in charts]\label{lem:orient-signInCharts}
	In the situation above let $\varphi$ be an orientation preserving chart around $x$
	and $\psi$ one around $h(x)$. Then
	\begin{equation*}
		\sign\det(\diff h_x)=\sign\det\left(\restr{\diff\left(\psi\circ h\circ \varphi^{-1}\right)}{\varphi(x)}\right)\,,
	\end{equation*}
	the right hand side being the determinant of a matrix in $\GL_n(\R)$.
\end{lemma}
\begin{proof}
	Put $A\coloneq \restr{\diff(\psi\circ h\circ\varphi^{-1})}{\varphi(x)}
	=\diff\psi_{h(x)}\circ \diff h_x\circ (\diff\varphi_x)^{-1}$ and write
	$e\coloneq e_1\wedge\dots\wedge e_n$. Since $\varphi$ is orientation preserving and
	$\Lambda^n(\diff\varphi_x)(\omega^\varphi_x)=e$, we have
	$\Lambda^n(\diff\varphi_x)(\omega_x)=a\cdot e$ with $a>0$, and likewise
	$\Lambda^n(\diff\psi_{h(x)})(\eta_{h(x)})=b\cdot e$ with $b>0$. Now
	\begin{equation*}
		\Lambda^n(\diff\psi_{h(x)})\circ\Lambda^n(\diff h_x)(\omega_x)
		=\Lambda^n(A)\circ \Lambda^n(\diff\varphi_x)(\omega_x)
		=a\det(A)\cdot e\,,
	\end{equation*}
	while the comparison element is sent to $b\cdot e$. As
	$\Lambda^n(\diff\psi_{h(x)})$ is an isomorphism it preserves the relation of lying
	in a common component, so the two agree precisely when $a\det(A)$ and $b$ have the
	same sign, i.e.\ when $\det(A)>0$.
\end{proof}

\begin{remark}[Orientation convention]\label{conv:orient-orientationOfTheSphere}
	Let $n\geq 1$. The bundle $\det(TS^n)$ is trivialisable, and $S^n$ is connected,
	so by Lemma \ref{lem:morse-OrientationsInAPointInduceOrientationsOfBundle} there
	are exactly two orientations of $S^n$, each determined by its restriction to a
	single fibre. We fix the one whose restriction to $\det(T_NS^n)$ is the component
	containing $\Lambda^n\left(\restr{\diff \pi_S}{N}\right)^{-1}(e_1\wedge\dots\wedge e_n)$,
	where $\pi_S$ is the stereographic projection from the south pole of Definition
	\ref{def:orient-fromRelaSpaceToTheSphere} and $N$ is the north pole.
\end{remark}

\begin{remark}
	With this choice $\pi_S$ is an orientation preserving chart on all of
	$S^n\setminus\{S\}$. Indeed, $\omega^{\pi_S}$ and $\restr{\omega}{S^n\setminus\{S\}}$
	are both nowhere-vanishing sections over the connected space $S^n\setminus\{S\}$,
	so the sign of their quotient is constant by the argument in the proof of Lemma
	\ref{lem:morse-OrientationsInAPointInduceOrientationsOfBundle}, and it is positive
...
\begin{proof}
	...
	rule together with Lemma \ref{lem:orient-signInCharts}.
\end{proof}
