...
\end{definition}
\begin{remark}
	The requirement of second countabilitys together with Hausdorff ensures that we have partitions of unitiy suborinate to an open cover of the manifolds. In the smooth case we can assume the continuous maps of the partition to be smooth. 
\end{remark}
...
\begin{definition}[Smooth Functions] \label{def:vb-smoothFunctions}
	...
	\begin{align*}
		\mathcal{E}(U)=\{f :  U \to \R \text{ continuous}~|~ f \text{ is differentiable of class }\infty \} .
	\end{align*}
\end{definition} 
...
\end{proof}
\begin{definition}
	If $p\in M$ is fix, $f\in \mathcal{E}(U)$ and $g\in \mathcal{E}(U')$ such that $p\in U\cap U'$ we say that $f$ and $g$ have the same \textbf{germ in $p$}, if there is another open neighbourhood $W\subseteq U\cap U'$ of $p$ such that $\restr{f}{W}=\restr{g}{W}$. This defines an equivalence relation $\sim_p$. An equivalence class $s$ of local functions around $p$ is called a \textbf{germ in $p$}. We write $s=f_p$, if $s=[f]$ with $f\in \mathcal{E}(U)$. 
	We write 
	\begin{align*}
...
\begin{definition}[Vector Bundles]\label{def:vb-vectorBundles} Let $\mathbb{K}$ be a field.
	A \textbf{$\mathbb{K}$ vector bundle} of rank $m$ is a surjection 
	\begin{equation*}
		\pi: E\to X
	\end{equation*}such that $E$ and $X$ are topological spaces and $\pi$ is continuous. Furthermore $\pi$ satisfies:
	 \begin{enumerate}
	 	\item For all $x\in X$, $\pi^{-1}(x)$ carries the structure of an $m$-dimensional $\mathbb{K}$-vector space.
	 	\item For all $x\in X$, there exist a neighbourhood $U\subseteq X$ and a homeomorphism 
		 	\begin{equation*}
		 		\psi: \restr{E}{U}\coloneq \pi^{-1}(U) \to U\times \mathbb{K}^m \,,
		 	\end{equation*}
		 	that is compatible in the following sense: Let $\pi_1$ denote the projection $ U\times \mathbb{K}^m \to U$ onto the first factor. Then 
		 	\begin{equation*}
		 		\pi_1 \circ \psi=\pi  
		 	\end{equation*} and 
		 	\begin{equation*}
		 		\psi_x: E_x\coloneq \pi^{-1}(x)\to \{x\}\times \mathbb{K}^m 
		 	\end{equation*} is a vector space isomorphism. 
	 \end{enumerate}
	 We call $E$ the \textbf{total space}, $X$ the \textbf{base space}, $E_x$ the \textbf{fibre over $x$} and $\psi$ a \textbf{local trivialisation}.
\end{definition}
\begin{remark}
	In the context of differential geometry we will mostly consider $\R$ vector bundles. However, the K-theory that we will develop will depend on $\C$ vector bundles.
\end{remark}
\begin{definition}
...
\begin{definition}[Pullback of Vector Bundles] \label{def:vb-pullbackOfVerctorBundles}
...
\end{example}
\begin{remark}\label{rem:vb-restrictionOfBundles}
	We will often omit the notation of the projection and denote a vector bundle by its total space. The restriction $	\restr{\pi}{Y}:\pi^{-1}(Y)\to Y$ is often denoted by $\restr{E}{Y}$. The restriction may equivalently be defined by restricting the domain and codomain of the projection. 
\end{remark}
\begin{comment}
	...
	\begin{definition}[Family of Vector Spaces] \label{def:vb-familiyOfVectorSpaces}
		...
		\begin{enumerate}[(i)]
			...
			\item a finite dimensional vector space structure on each fibre $E_x:=\pi^{-1}(x)$ that is compatible with the subspace topology on $E_x\subseteq E$.
		\end{enumerate}
	...
	\begin{remark} Let $\pi: E\to X$ be a family of vector spaces.
		By locality of the definition, for all open $U\subseteq X$ the restriction $\restr{\pi}{U}:\pi^{-1}(U)\to U$ defines a new family of vector spaces. With this we can also have sections of the restricted bundle and the collection of those is denoted by $\Gamma(U)$.
	\end{remark}
...
\begin{definition}[Homomorphism of families of vector spaces]\label{def:vb-homomorphismOfFamiliesOfVectorSpaces}
	A \textbf{homomorphism} from one family $\pi:E\to X$ to another family $\Tilde{\pi}:F \to X$ with the same base space is a continuous map $\phi: E\to F$ such that:
	\begin{enumerate}[(i)]
	...
	\end{enumerate} In the case that $\phi$ is bijective (and thereby a point-wise vector space isomorphism) and has a continuous inverse we call it an \textbf{isomorphism} and call $E$ and $F$ isomorphic. 
\end{definition}
\end{comment}



\begin{remark}\label{rem:vg-pullbackInducesBundleMap}
...
\begin{remark}
	Let $\pi:E\to X$ be a vector bundle. Then the space of sections $\Gamma(E)$ is a (in general, infinite-dimensional) $\R$-vector space. Furthermore, and more relevant to the purposes of this thesis, we view $\Gamma(E)$ (and $\Gamma(U)$ for open subsets) as a $C(X)$ (or $C(U)$) module.  The latter denotes the ring of continuous functions from $E$ (or $U$) to $\R$. 
\end{remark}
...
Then a section is called smooth if it is a smooth map of manifolds. A family of smooth local sections $(s_1,\dots, s_r)$ with $s_i\in \Gamma(U)$ is called a \textbf{local frame} if they form a basis of the $C(U)$-module $\Gamma(U)$. Such a family is obtained from $\Phi$ by defining $s_i(p)\coloneq \Phi^{-1}(p,e_i)$. Similarly, from a local frame we can obtain a local trivialisation by defining:
\begin{equation*}
	\Phi: \pi^{-1}(U) \to U\times \R^r\,,\quad v=\sum_{i=1}^{r} \lambda_i s_i(\pi(v)) \mapsto (\pi(v),\lambda_1,\dots, \lambda_r)\,.
\end{equation*}
\end{definition}
\begin{example}
The local trivialisations $\partial_i$ of the tangent bundle of a smooth manifold $M$ form a local frame and make the tangent bundle into a smooth vector bundle.
\end{example}
...
\end{lemma}
\begin{proof}
	To show that it is an isomorphism we need a continuous inverse, equivalently, we need to show that $\phi$ is a homeomorphism. First of all we can deduce, that $\phi$ is bijective:
	Assume it wasn't injective, then there would be a fibre in which $\phi_x$ is not injective. Assume that it wasn't surjective. Then there would be a $w\in F$ that has no preimage which means that in the fibre containing $w$ there is no isomorphism. Thus, there is a unique inverse and it remains to verify that it is continuous, which is a local condition. Therefore we can assume that $E$ and $F$ are product bundles. Then we have a continuous map $\Phi:X\to \Hom(V,W)$. Furthermore, since all $\phi_x$ are isomorphisms, we can restrict the image to $\Iso(V,W)$ which is an open subset of $\Hom(V,W)$. Now we can define the map $x\mapsto \Phi(x)^{-1}$ which is continuous since taking inverses is continuous. By the corollary above, we obtain a continuous map $\psi:F\to E$ by sending $(x,w)\mapsto (x,\Phi(x)^{-1}(w))$, which is the inverse. 
\end{proof}
\begin{example}
	Let $M$ be a smooth manifold of dimension $m$, $\phi:U\to V$ a chart and $\tilde{\phi}:TU\to V\times \R^m$ be the corresponding bundle chart. Then it follows from the lemma, that $TU$ is isomorphic to $V\times \R^m$. This is verified by observing that $\partial_i(p)$ is a basis of $T_pU$ for all $p\in U$. In this case $\tilde{\phi}\left(\partial_i\right)=e_i$ giving us isomorphisms in each fibre.
\end{example}
\begin{definition}[Sub-bundles]\label{def:vb-subBundles}
	Let $\pi:E\to X$ be a vector bundle. A \textbf{sub-bundle} is a subset of $E$ which is a vector bundle in the induced topology and fibrewise vector space structure.
\end{definition}
\begin{definition}[Mono- and Epimorphisms]\label{def:vb-monoAndEpimorphisms}
	A homomorphism $\phi: E\to F$ is a \textbf{monomorphism} if all $\phi_x$ are monomorphisms (i.e. injective). Respectively, it is called an \textbf{epimorphism} if all $\phi_x$ are epimorphisms (i.e. surjective).
\end{definition}
\begin{corollary}
	If $F$ is a sub-bundle of $E$, then the inclusion $\phi:F\to E$ is a monomorphism.
\end{corollary}
\begin{lemma} \label{lem:vb-monosInduceSubBundles}
	If $\phi:E \to F$ is a monomorphism, then $\phi(E)$ is a sub-bundle and $\phi:E\to \phi(E)$ is an isomorphism.
\end{lemma}
\begin{proof}
	Once we know that $\phi(E)$ is a sub-bundle, the map $\phi:E\to \phi(E)$ is a bijective homomorphism and fibrewise an isomorphism, hence an isomorphism by Lemma \ref{thm:vb-isomorphicVectorBundles}. So we need to check that $\phi(E)$ is a sub-bundle, which is a local property. Thereby we can assume that $E$ and $F$ are product bundles $E=X\times V$ and $F=X\times W$. Let $x\in X$ and let $W_x\subseteq W$ be a subspace complementary to $\phi_x(V)$. Then $G\coloneq X\times W_x\subseteq F$ and we define
	\begin{align*}
		\theta:E\oplus G &\to F\\
		(a,b)&\mapsto \phi(a)+i(b)\quad \text{ where $i$ denotes the inclusion }G\hookrightarrow F \, .
	\end{align*} In the fibre over $x$, the map $\theta_x$ is an isomorphism and thereby there exists an open neighbourhood $U$ of $x$ such that $\restr{\theta}{U}$ is an isomorphism. That is because $\Iso(V\oplus W_x,W)$ is open in $\Hom(V\oplus W_x,W)$. Since $E$ is a sub-bundle of $E\oplus G$ we have that $\restr{\theta(E)}{U}=\restr{\phi(E)}{U}$ is a sub-bundle of $\restr{F}{U}$.
\end{proof}
\begin{corollary}\label{cor:vb-subBundlesAreLocalSummands}
	In the proof, we have shown more: locally a sub-bundle is a direct summand. Furthermore, we have shown that the set of points $x$ where $\phi_x$ is a monomorphism, is open.
\end{corollary}
\begin{definition}[Quotient Bundle]\label{def:vb-quotientBundle}
	If $F$ is a sub-bundle of $E$ we define the \textbf{quotient bundle} $E\slash F$ as the union of the spaces $E_x\slash F_x$ equipped with the quotient topology.
\end{definition}

\begin{corollary}
	The quotient bundle is a vector bundle, because locally $F$ is a direct summand and thereby $E\slash F$ is locally trivial.
\end{corollary}
\begin{lemma}\label{lem:vb-quotiendBundlesOfSmoothSubbundlesAreSmooth}
	Quotiend bundles of smooth subbundles are smooth.
\end{lemma}
\begin{proof}
Let $F \subseteq E$ be a smooth subbundle of rank $k$, where $E$ has rank $n$ and $M$ is the base manifold. We need to check that the transition functions consist of smooth maps into the structure group. First, we will show that there are smooth local frames of $E$ that restricts to a smooth local frame of $F$:

Let $p\in M$. Since $F$ is smooth there exists a local frame $(s_1,\dots ,s_k)$ of $F$ arround $p$. In particular, the $s_i$ are smooth local sections into $E$ arround $p$. Let $(t_1,\dots, t_k)$ be a smooth local frame of $F$ near $p$. (without restrictions over the same domain as the sections $s_i$) Now we make use of steinitz exchange lemma to choos $n-k$ many $t_i$ (Without restrictions $t_{k+1},\dots,t_n$) such that 
\begin{equation*}
	\big(s_1(p),\dots s_k(p),t_{k+1}(x),\dots,t_n(p))
\end{equation*} is a basis of $E_p$. Being a basis over a point $q\in M$ near $p$ is equivalent to the invertability of the matrix where the columns are the sections at $q$, and since invertability (or rather nonzero determinant) is an open condition, there exists a neighbourhood, such that the family 
\begin{equation*}
	\big(s_1,\dots s_k,t_{k+1},\dots,t_n)
\end{equation*} is a local frame, whose first $k$ sections span $F$.  By Definition
    \ref{def:vb-framesFromTrivialisations} it induces a smooth
    trivialisation $\Phi_U$ of $E$ with
    $\Phi_U\big(\restr{F}{U}\big)=U\times\R^k\times\{0\}$, and
    fibrewise passing to the quotient
    $\R^n/(\R^k\times\{0\})\cong\R^{n-k}$ yields a trivialisation
    $\bar\Phi_U$ of $\restr{(E/F)}{U}$.

	Now we can cover $M$ by such adapted trivializations. Their transition functions  $g_{i,j}:U_i\cap U_j\to\GL(n,\R)$ are smooth and pointwise preserve $\R^k\times\{0\}$: for $v\in\R^k\times\{0\}$ we have
$\Phi_i^{-1}(p,v)\in F_p$ since $\Phi_i$ is adapted, and hence
$\Phi_j(\Phi_i^{-1}(p,v))\in\{p\}\times\R^k\times\{0\}$ since $\Phi_j$
is adapted. Hence they have block form
\begin{equation*}
	g_{i,j}=\begin{pmatrix} A_{i,j} & B_{i,j}\\ 0 & C_{i,j}\end{pmatrix}\,,
\end{equation*}where all coordniate entries and hence all blocks smoothly depend on the basepoint. Finally, the transition functions of the quotient with respect to the trivialization $\bar\Phi_U$ of $\restr{(E/F)}{U}$ consist of the smooth maps $p\mapsto C(p)=(C_{i,j})_{i,j}(p)$ into the structure group $\GL(n-k,\R)$.
\end{proof}
...
\end{definition}
	...
	\begin{example}[Cotangent bundle]
		Let $M$ be a smooth manifold of dimension $m$. Then we can define the \textbf{cotangent bundle} $T^*M$ to be the dual of the tangent bundle. A local frame of said bundle is given by the fibrewise dual of the frame $(\partial_i) $. Sections into the tangent bundle are called \textbf{vector fields} and sections into the cotangent bundle are called \textbf{one-forms}.  
	\end{example} 
...
\end{remark}
\begin{proof}
	We start by assuming that we have an atlas with structure group $\O(m)$. In Definition \ref{def:vb-framesFromTrivialisations} we saw that a covering of local trivialisations is equivalent to a covering of smooth frames. Now in a single trivialisation
	\begin{equation*}
...
\begin{align*}
	...
	\end{array} 
\end{align*} where $\cdot$ denotes the standard scalar product. This gives us locally a Riemannian metric. If we do this for all trivialisations we have to make sure, that the result is well defined. So assume that two trivialisations $\Phi$ and $\Psi$ overlap in $V$. The structure group is $\O(m)$, $\Phi=R\Psi$ for some function 
\begin{equation*}
	R: V\to \O(m)\,.
\end{equation*} Let $p$ be in said overlapping and $(\nu_p,\xi_p)$ be from $T_pM\times T_pM$. Then since $R$ is orthogonal we can calculate the independence from the trivialisation:
\begin{equation*}
...
\begin{lemma}[Local Description of the Gradient]\label{lem:vb-localDescriptionOfGradient}
	Let $(M,g)$ be a smooth Riemannian manifold of dimension $m$ and $\phi:U\to V$ be a chart. Then the gradient has the local form:
	\begin{align*}
...
\begin{definition}[A Dynamical System]
	Let $M$ be a smooth manifold. A \textbf{dynamical system on $M$} (sometimes also called a \textbf{flow on $M$}) is given by a smooth map $\phi:\Omega \to M$ such that:
	\begin{enumerate}
