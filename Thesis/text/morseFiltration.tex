...
\begin{remark}\label{rem:morseFiltr-morphismToCanonicalIntegers}
	...
	\end{equation*} Furthermore, this map is given by mapping a generator of $\tilde{K}^{-\lambda_q-2i}(N_q/L_q)$ to one, and hence uniquely determined, provided we have a canonical way to choose a generator of $\tilde{K}^{-\lambda_q-2i}(N_q/L_q)\cong \tilde{K}^{-\lambda_q-2i}(\overline{T^u_qM})$, where the isomorphism is natural and described in Remark \ref{rem:morseFiltr-criticalPointsAsIsolatedCompactInvariantSubsets}. 
	
	Hence, if $T^u_qM$ is oriented, Corollary \ref{cor:orient-vectorspaceOrientationInducesKtheoreticOrientation} gives us just what we need, making the map canonical. The generator is then given by pulling back the generator of $K^{-\lambda_p}(\overline{T^u_qM})$ along the map of pairs:
	\begin{align*}
	\kappa_q\coloneq R_0\circ \restr{\diff \phi^{-1}}{0}\circ s\circ pr_u \circ \phi	\,:\,(\phi^{-1}( D^{s }_\epsilon\times  D^{u }_\epsilon),\phi^{-1}( D^{ s}_\epsilon\times \partial D^{u}_\epsilon))  \to (\overline{T^u_qM},\{\infty\})
	\end{align*} where $pr_u$ is the projection onto the unstable coordinates and $s$ is the streching $x\mapsto x\cdot (\frac{1}{\epsilon}-\norm{x}-\frac{1}{\epsilon})$ with $\restr{\diff s}{0}=\frac{1}{\epsilon}\Idm$.
\end{remark}
...
\begin{equation*}
	\Delta^{k-1} \coloneq \Big(\sum_{q\in\Crit_{k}}\sum_{p\in \Crit_{k-1}} c_p\circ \big(b\circ \delta_{triple}^{p,q}\circ i_q\big)\Big)_{\#}: K^{-(k-1)}_{\#}(\mathcal{IP}_{k-1}) \to K^{-k}_{\#}(\mathcal{IP}_{k} )\,.
\end{equation*} 

Due to the naturality of the triple connecting homomorphism, the inclusion and the collapsing, any list of topological triples $(X,Y,Z)$ such that $(X,Y)$ is a regular index pair of $\{q\}$, $(Y,Z)$ of $\{p\}$ and $(X,Z)$ of $W(q\to p)\cup \{q\}\cup \{p\}$ for all $q\in \Crit_k(f),p\in \Crit_{k-1}(f)$ gives rise to the same morphism of canonical isomorphism classes.
\end{example}	



\begin{theorem}\label{thm:morseFilt-commutingMorsecoboundary}
	...
	\begin{equation}
		\begin{tikzcd}\label{eq:morseFilt-commutingDiagramTheorem}
			C^{k-1}(M,\mathfrak{A},K^l(pt)) \arrow[r,"\partial^{k-1}"] \arrow[d] 
			& 
			C^{k}(M,\mathfrak{A},K^l(pt)) \arrow[d]
			\\
			K^{-(k-1)-l}_{\#}(\mathcal{IP}_{k-1}) \arrow[r,"\Delta^{k-1+l}"]& K^{-(k)-l}_{\#}(\mathcal{IP}_{k})
		\end{tikzcd}
	\end{equation}
\end{theorem}
Before we can proof this theorem we need the following Lemma:

\begin{definition}[Definitions and Setup]\label{def:morseFilt-setupCommutingDiff}
	We work in a compact, riemanian, smooth Manifold $(M,g)$ of dimension $m$ together with a Morse-Smale funktion 
	\begin{align}
		f: M\to \R
	\end{align} and orientations $\mathfrak{o}$ of all unstable tangend spaces.
	We assume that $q$ is a critical point of index $k$ and $p$ is a critical point of index $k-1$. we define $f(q)=b$ and $f(p)=a$ and assume that there is no critical point in $f^{-1}(a,b)\subseteq M$. We define the sub and superlevelsets 
	\begin{align}
	...
	\begin{align}
		...
		S(q\to)& \coloneq W(q\to)\cap f^{-1}(c)\				\\
		D_q & \coloneq W(q\to)\cap M_c\,.							
	\end{align}
	
	
	Furthermore, we assume $W(q\to)$ to be oriented with respect to the orientation of $T^u_qM$ as in Proposition \ref{prop:morse-OrientedUnstableSpaces}. Let $Q^p\coloneq \restr{TM}{W(\to p)}\Big/ TW(\to p)$ be orienbted with respect to $T^u_pM$ as in Proposition \ref{prop:morse-orientTheQuotient}.
	For an oriented real vectorspace $V$ of dimension $n$, let $\beta_V\in K^{-n}(\overline{V})$ denote its K-theoretic orientation from Corollary \ref{cor:orient-vectorspaceOrientationInducesKtheoreticOrientation}. For an index pair $(N,L)$ of $\{p\}$ let $\alpha^{(N,L)}_p\in K^{-\lambda_p}(N,L)$ denote the canonical generator from Remark \ref{rem:morseFiltr-morphismToCanonicalIntegers}, i.e.\ the pullback of $\kappa_p^*(\beta_{T^u_pM})$ along the homotopy equivalence of Theorem \ref{thm:morseFiltr-homotopyEquivalenceOfIndexPairs} from $(N,L)$ to the standart index pair and $\kappa_v= \kappa_q\coloneq R_0\circ \restr{\diff \phi^{-1}}{0}\circ s\circ pr_u \circ \phi$.
	\begin{center}
	...
	\end{center}
\end{definition}
\begin{lemma}[]\label{lem:morseFilt-npIsTubular} In the setting of Definition \ref{def:morseFilt-setupCommutingDiff}, we claim that 
	\begin{enumerate}
...
\begin{proof}[Proof of Theorem \ref{thm:morseFilt-commutingMorsecoboundary}]
	For all $l\in \Z$ we get the commuting square 
	\begin{center}
		\begin{tikzcd}
			K^{-(k-1)-l}_{\#}(\mathcal{IP}_{k-1}) \arrow[r,"\Delta^k"]\arrow[d,"b"]& K^{-(k)-l}_{\#}(\mathcal{IP}_{k})\arrow[d,"b"]\\
			K^{-(k-1)-l-2}_{\#}(\mathcal{IP}_{k-1}) \arrow[r,"\Delta^k"]& K^{-(k)-l-2}_{\#}(\mathcal{IP}_{k})
		\end{tikzcd}
	\end{center}
	Hence, we only need to proof the original statement for $l=0,1$.
	For $l=1$ the bottom row of the diagram \ref{eq:morseFilt-commutingDiagramTheorem} is trivial, as the connected simple systems $\mathcal{IP}_k$ consists of spaces that are homotopically equivalent to $k$-spheres and we know by \ref{cor:kTheo-kGroupsOfTheSpheres} that $K^{-k-1}(S^k)\cong\tilde{K}(S^{2k+1})=0$. The same argument works for the image of $\Delta^k$ for $l=1$. So for the rest of the proof we assume $l=0$. Now since $N_p$ is a tubular neighbourhood of $W(\to p)\cap M^c$ we know that $N_p\cap S(q\to)$ consists of finitly many components $V_j$ and each component has one $\{x_j\}=W(q\to p)\cap V_j$. In fact, the proof of Lemma \ref{lem: The Main Lemma Np is tubular} shows that the $V_j$ are fibers of the normal neighbourhood of $W(\to p)$ inside $W(q\to)$ and hence homeomorphic to $\R$.\todo{referenz!}
	Now we take the tiple $(C,B,A)$ from Lemma \ref{lem: The Main Lemma Np is tubular}. We have a map of triples: 
	\begin{align*}
			\left( ~W(q\to)\cap N_q ~,~S(q\to)~,~S(q\to ) \setminus \bigcup\limits_j\inti(V_j) ~\right) \to (C,B,A)
		\end{align*} giving us a commuting diagramm (the square bracets for the input of the K funktors $K[-]$ are choosen for readability and have no deeper meaning)
		\begin{fdiagram}
			\begin{tikzcd}
				{K^{-\lambda_p}\left[ B,A \right]} \arrow[r, "\delta_{triple}^1"] \arrow[d, "{i_{B,A}^*}"]                                                            & {K^{-\lambda_q}\left[C, B \right]} \arrow[d, "{i_{C,B}^*}"] \\
				{K^{-\lambda_p}\left[ S(q\to),S(q\to)\setminus \bigcup\limits_j \inti(V_j) \right]} \arrow[r, "\delta_{triple}^2"] \arrow[d, "i_j^*"', shift right=2] & {K^{-\lambda_q}\left[W(q\to)\cap N_q,S(q\to) \right]}       \\
				{K^{-\lambda_p}\left[ V_j,\partial V_j \right]} \arrow[ru, "f^j"'] \arrow[u, "c_j^*"']                                                  &                                                            
			\end{tikzcd} 
		\end{fdiagram}
		Now $i^*_{C,B}$ is an isomorphism, because the collapsing of the tubular neighbourhood $N_q$ induces a homotopy equivalence
		\begin{equation*}
			\frac{N_q\cup N_p}{N_p\cap L_q}		 	\, \cong \, 
			 \frac{N_q}{L_q}		  				\; \to   \; 
			\frac{W(q\to)\cap M_c}{L_q\cap W(q\to)} \, \cong \, \frac{W(q\to)\cap N_q}{S(q\to)}
		\end{equation*} that is inverse to the inclusion (once the pairs are viewed as pointed quotients). For the rest of the proof we will often argue in similar ways by identifying pairs and there pointed spaces given by quotiening the subspace. This is justified, as the K-theory of a pair is defined to be the K theory of said quotiont, and this helps us in the proof as we can write down maps of pointed spaces, that would not be continuous as maps of pairs.
		Due to the quotient
		\begin{equation*}
			S(q\to)\Big/S(q\to)\setminus \bigcup\limits_j \inti(V_j)\cong \bigvee_j \big(V_j/\partial V_j\big)\,
		\end{equation*}being a pointed sum, the map $\delta^2_{triple}$ is a sum of the functions $f^j\circ i_j^*$ where the $f^j:K^{-\lambda_p}\left[ V_j,\partial V_j \right]\to K^{-\lambda_q}\left[W(q\to)\cap N_q,S(q\to) \right]$ are defined to be $\delta^2_{triple}\circ c_j^*$.
		Now we will inspect said composition more closely, for which we need to dive into the definition of the triple connecting homomorphism, which was given by the composition of the connecting homomophism of the pair after the pullback alng the inclusion:
\begin{fdiagram}
\begin{adjustbox}{max width=\textwidth}

\begin{tikzcd}
{K^{-\lambda_p}\left[ V_j,\partial V_j \right]} \arrow[d, "c_j^*"] \arrow[rd, "f^j", bend left]                                                                 &                                                        \\
{K^{-\lambda_p}\left[ S(q\to),S(q\to)\setminus \bigcup\limits_j \inti(V_j) \right]} \arrow[d, "i_1^*"] \arrow[r, "\delta_{triple}"]                             & {K^{-\lambda_p+1}\left[W(q\to)\cap N_p,S(q\to)\right]} \\
{K^{-\lambda_p}\left[ S(q\to) \right]=K^{-\lambda_p+1}\left[S\wedge\big( S(q\to)\big) \right]} \arrow[ru, "\delta_{pair}", bend right] \arrow[d, "\theta^{-1}"] &                                                        \\
{K^{-\lambda_p+1}\left[ W(q\to)\cap N_p \cup C\left( S(q\to)\right),W(q\to)\cap N_p \right]} \arrow[d, "m^*"]                                                   &                                                        \\
{K^{-\lambda_p+1}\left[ W(q\to)\cap N_p \cup C\left( S(q\to)\right)\right]} \arrow[ruuu, "(\pi^{*})^{-1}"', bend right]                                         &                                                       
\end{tikzcd}

\end{adjustbox}
\end{fdiagram}
If we already start by identifying $K^{-\lambda_p}\left[ V_j,\partial V_j \right]$ with $K^{-\lambda_p+1}\left[ S \wedge(V_j,\partial V_j) \right]$\todo{def wedge of a pair!} we have that the composition
\begin{equation*}
	(\id \wedge l_1)^*\coloneq i_1^*\circ c_j^*:K^{-\lambda_p+1}\left[ S \wedge(V_j,\partial V_j) \right] \to K^{-\lambda_p+1}\left[S\wedge\big( S(q\to)\big) \right]
\end{equation*} is induced by the collapsing map 
\begin{equation*}
	l_1:(S(q\to)) \to (V_j\big/ \partial V_j)\,.
\end{equation*} 
This brings us into the following setup: 

\begin{fdiagram}
	\begin{adjustbox}{max width=\textwidth}
\begin{tikzcd}
{K^{-\lambda_p+1}\left[ S \wedge(V_j,\partial V_j) \right]} \arrow[rr, "f^j"] \arrow[rd, "(id\wedge l)^*"]                  &                                                                                                                 & {K^{-\lambda_p+1}\left[W(q\to)\cap N_p,S(q\to)\right]}                         \\
                                                                                                                            & {K^{-\lambda_p+1}\left[S\wedge\big( S(q\to)\big) \right]} \arrow[ru, "\delta_{pair}"] \arrow[rd, "r^*", dashed] &                                                                                \\
{K^{-\lambda_p+1}\left[\overline{ \R^{\lambda_p +1}}\right]} \arrow[uu, "f_p"] \arrow[rr, dashed] \arrow[ru, "s^*", dashed] &                                                                                                                 & {K^{-\lambda_p+1}\left[\overline{ \R^{\lambda_p +1}}\right]} \arrow[uu, "f_q"]
\end{tikzcd}
	\end{adjustbox}
\end{fdiagram}

The dashed arrows denote functions that we still need to find, to make the diagramm commute. The functions $f_p$ and $f_q$ will be given below. We will later see, that the notation of $s^*$ and $r^*$ is justified in the sence that those are topologically induced funktions. 
...
{K^{-\lambda_p+1}\left[ S \wedge(V_j,\partial V_j) \right]} \arrow[rr, "f^j"] \arrow[rd, "(id\wedge l)^*"]                  &                                                                                                                 & {K^{-\lambda_p+1}\left[W(q\to)\cap N_p,S(q\to)\right]}                         \\
                                                                                                                            & {K^{-\lambda_p+1}\left[S\wedge\big( S(q\to)\big) \right]} \arrow[ru, "\delta_{pair}"] \arrow[rd, "r^*", dashed] &                                                                                \\
{K^{-\lambda_p+1}\left[\overline{T_{x_j}W(q\to)}\right]} \arrow[uu, "f_p^*"] \arrow[rr, dashed] \arrow[ru, "s^*", dashed] &                                                                                                                 & {K^{-\lambda_p+1}\left[\overline{ \R^{\lambda_p +1}}\right]} \arrow[uu, "f_q^*"]
\end{tikzcd}
	\end{adjustbox}
\end{fdiagram}
...
Notice how by definietion of the reduced suspension, there is a natural homomorphism: $S\wedge(V_j\Big/\partial V_j)\cong ([-1,1]\times V_j \big/ \partial([-1,1]\times V_j))$
Now $V_j$ is the image of $\mathcal{N}_{x_j}\gamma^{W(q\to)}$\todo{notation einführen} under the tubular neighbouzrhood embedding $J$. And $\inti([-1,1])$ is homeomorphic to $\R$ via $x\mapsto \frac{-x}{x^2-1}$. This gives us a mapping 

\begin{equation*}
	f_p:S\wedge(V_j\Big/\partial V_j)\to \overline{\R\oplus \mathcal{N}_{x_j}^{W(q\to)}\gamma}
\end{equation*}


With this we can define the homomorphism $\tilde{f_p}: (I\times V_j \big/ \partial(I\times V_j)) \to \overline{ T_{x_j}W(q\to)}$ as the closure of the map $(x,v)\mapsto -\frac{-x}{x^2-1}\cdot\grad_{x_j}(f)+J(v)$ Then we define $f_p\coloneq \big(\overline{q_{\mathfrak{b}_q}}\circ \tilde{f^p}\big)^*$, where the map $g_{\mathfrak{b}_q}:\R^{\lambda_p+1}\to T_{x_j}W(q\to)$ is the inverse of the koordinate map $\kappa_{\mathfrak{b}_q}$ where $\mathfrak{b}_q$ is the basis of $T_{x_j}W(q\to)$ induced by Proposition $\ref{def:morse-signedFlowLines}$ that comes from the short exact sequence.

Let $(\alpha_p)_{\#}$ be a generator of $K^{\lambda_p}_{\#}(\mathcal{IP}_p)$ meaning that $(\alpha_p)_{\#}$ is a family of generators for all groups in $K^{\lambda_p}_{\#}(\mathcal{IP}_p)$ that transform into each other under the canonical group isomorphisms.
 Notice that $(\alpha_p)_{\#}$ is induced by any of its elements. Now lets fix $(\alpha_p)_{\#}$ to be the generator induced in Remark \ref{rem:morseFiltr-morphismToCanonicalIntegers} and hence given by the generator $\alpha_p=\kappa_p^*(\mathfrak{b}_q)$ of $K^{\lambda_p}(\phi^{-1}( D^{s }_\epsilon\times  D^{u }_\epsilon),\phi^{-1}( D^{ s}_\epsilon\times \partial D^{u}_\epsilon))$, where $\mathfrak{b}_q$ is the generator of $\tilde{K}^{\lambda_p}(\overline{T^u_qM})$ induced by the orientation of $T^u_qM$. Furthermore, let $\alpha_p^{(B,A)}\in (\alpha_p)_{\#}$ be the generator of $K^{-\lambda_p}\left[B,A\right]$. Let $\mathfrak{e}$ denote the generator of $K^{-\lambda_p+1}(\R^{\lambda_p+1})$ induced by the standart bases (and composed with a bott periodicity map).
Then $f^p(\mathfrak{e})=i_j^*\circ i^*_{B,A}(\alpha_p^{(B,A)})$
\end{proof}
