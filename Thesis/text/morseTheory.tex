...
\begin{definition}\label{def: Definition produce Morse datum}
	...
	\end{equation*} This results in the index of $(q,q')$ being at least two bigger then the index of $(p,p')$. Hence, if we want to count flow lines between two consecutive indices, either $p$ equals $p'$ or $q$ equals $q'$. In this case we have the moduli space
	\begin{equation*}
		...
		\end{cases}
	\end{equation*}
	Now, we want to analyse how the splitting affects the counting of flow lines. For this denote the flow lines $\tilde{\gamma}\in \mathcal{M}\left((q,q')\to (p,p')\right)$, $\gamma \in \mathcal{q\to p}$ and $\gamma'\in \mathcal{M}(q'\to p')$ such that $\tilde{\gamma}=\gamma\times \gamma'$.
	...
	\begin{center}
		...
		\end{tikzcd}
	\end{center}
	This sequence however is graded in the sence that all spaces (with exception of the $0$) are direct sums and all maps respect them, meaning it is the direct sum of the two short exact sequences:
	\begin{equation}\label{eq:morse-firstExactSequence}
		\begin{tikzcd}
			0 \arrow[r]& T{\gamma} \arrow[r]&\restr{TW\left(q\to \right)}{{\gamma}}\arrow[r]&
	...
	\begin{equation}\label{eq:morse-secondExactSequence}
		...
		\end{tikzcd}
	\end{equation}
	Now, we have the two orientations of $\restr{TW\left((q,q')\to \right)}{\tilde{\gamma}}$: The first one comes from the orientation of $T_{(q,q')}^uM\times M'$ and is given as the direct sum of the orientations of $T_q^uM$ and $T_{q'}^uM'$ and thereby represented by the product of two nonvensinging sections into the determinant bundle $s_q \otimes s_{q'}$( Remember that the determinant of a direct sum is the tensor product of the determinants; This is also true for bundles). The other way of orienting comes from the exact sequences. Let $-\grad\wedge s_p$ denote a represantative of the orientation induced by the sort exact sequence (\ref{eq:morse-firstExactSequence}) and similar denote the orientation induced in the middle of the second short exact sequence \ref{eq:morse-secondExactSequence} by $-\grad'\wedge s_{p'}$.  By definition, the compatison of $s_p$ with $-\grad\wedge s_p$ gives the sign $\epsilon{\gamma}$ and similar the comparison of $s_{q'}$ with $-\grad'\wedge s_{p'}$ gives the sign $\epsilon(\gamma')$. For ease of notation we assume that the sections live in the right space (and we omit the needed inclusions or lifts to pair them with a $\wedge$.)

	Then in the product, the induced orientation from the short exact sequence is defiend by a representative of the form:
	\begin{equation*}
		(-\grad \wedge -\grad')\wedge ( s_p\wedge s_{p'})\,.
	\end{equation*}

	Hence we need to compare the two orientations:
	\begin{equation*}
		s_q \wedge s_{q'} \quad \text{ with }\quad (-\grad\wedge -\grad' )\otimes ( s_p\wedge s_{p'})
	\end{equation*}



	
	For this we go through our classical procedure given in the definition \ref{def: Morse homology}. Assume that $q'=p'$, and let $\gamma$ be an element of $\mathcal{M}(q \to p)$ with $x\in \gamma$. Then $(\gamma,q')\in \mathcal{M}\left((q,q'),(p,q')\right)$. Now let $(b,b')$ be a basis of  
	\begin{equation*}
