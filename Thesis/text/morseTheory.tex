...
\begin{theorem}[Morse Lemma]\label{thm:morse-morseLemma}
	Let $p$ be a critical point of index $k$ of the Morse function $f:M\to \R$. There exists a centered chart $\phi:U\to \R^m$ around $p$ such that 
	\begin{equation*}
...
\begin{lemma}\label{lem:morse-jabobianOfTheGradient}
	...
	\end{equation}where $g^{ij}$ is defines as in definition \ref{def:vb-localDescriptionOfGradientGij}.
\end{lemma}
\begin{proof}
...
\end{theorem}
\begin{proof}
	I will summarize the proof given in Chapter 4.3 \cite{banyaga2004lectures}.  We work in the discrete setting meaning that we take the negative gradient flow $\phi_t$ for a fixed positive $t$ and the limit-behaviour of the flow is captured by $\lim_{n\to \infty}\phi_t^n$ instead of the $t$-limit. First, on shows the splitting of spaces locally in a small neighbourhood $U$ arround a critical point $p$ and then extends the splitting globally. Arround $U$ we already know by remark \ref{rem:morse-jacobianOfTheFlowInRefinedMorsechart} that the tanged space $T_pM$ splits into $T^s_pM\oplus T^u_pM$ such that the differential of the flow is a contraction arround $T^s_pM$ and a expansion in $T^u_pM$. We can now map a small neighbourhood $U$ arround $p$ into $T^pM$ using the map $\Psi:U\to T_pM$ given by a refined morse chart and the identification of $\R^n$ with $T_pM$ via $e_i\mapsto \partial_i(p)$, where tha latter uses the same refined morse chart. Now inspect the image of the stable manifold $W(\to p)$ under said map. This looks to be a small pertubation of the subspace $T^s_pM$ and the local stable manifold theorem supports that intuition: 
	...
	\end{proof}
	We can now define $\theta \coloneq \Psi \circ \phi \circ \Psi^{-1}$. The local (un-)stable manifold theorem now tells us that $W^s_r(\theta)=\Psi(W(\to p)\cap U)$ is the graph of a smooth function $g: T^s_pM\to T^u_pM$. Hence the projection $\pi$ of said graph to its domain gives us a chart of the stable manifold arround $p$. A whole atlass of $W(\to p)$ is obtained by $U^n\coloneq \phi^{-n}(U)$ which covers the whole $W(\to p)$ and with this  the maps $\chi^n:\pi\circ \Psi \circ \phi^n$ give us an atlass. 
	The smooth embeddings ar obtained as follows: We define $h\coloneq \chi \circ \phi \circ \chi^{-1}$ and $B\coloneq W(\to p)\cap U$. Now $\diff \big(\restr{\phi_p}{T^s_pM}\big)$ having an operator norm less then one translates to $\diff h_p$ as they are similar as matricies. Let $0<\alpha<1$ and $B_0\subseteq B$ be a ball centered at $p$ such that $\norm{\diff h_x}\leq \alpha$ for all $x\in B_0$. Then $\restr{h}{B_0}h$ is a contraction, which can be extendet to a global contraction $\tilde{h}:T^s_pM\to T^s_pM$. Now we define the embedding:
	\begin{align*}
		E^s:T^s_pM&\to W(\to p) \\
		x\mapsto \phi^{-n}\circ \chi^{-1}\circ \tilde{h}^n(x)\quad n \text{such that }\tilde{h}^n(x)\in B_0 \,.
	\end{align*} All required properties and the well definition of $E^s$ is now standart calculations. The stable manifold theorem follows from the unstable one by replacing $f$ with $-f$. 
\end{proof}
...
\end{theorem}
\begin{proof}A proove is given in chapter 2.8 in \cite{Perko2001}.
\end{proof}
...
\begin{proof} 
	...
	\end{align*}
	The details of the proof are exactly the same as the standart (un-)stable manifold theorem.
\end{proof}
...
The idea of Morse homology is now to find an invariant under manipulations of the Morse Smale function. Obviously neither the number of flow critical points nor the number of flow lines stays invariant. However certain linearcombinations stay invariant, when wone establishes a sophisticated way to count flow lines. For this count we want to introduces signes to the flow lines between critical point with index one apart.
...
\begin{definition}\todo{das muss NACH die definition von Orientierung}
	Let $f:M\to \R$ be a Morse-Smale function on a Riemannian manifold $(M,g)$. Furthermore let $\mathfrak{o}$ be an choice of orientations for all unstable Tangendspaces ($T_q^uM$ for all $q\in \Crit(f)$). The tuple $\mathfrak{A}\coloneq (g,f,\mathfrak{o})$ is a \textbf{Morse datum}.
\end{definition}
\begin{definition}[Orientation]
	Let $V$ be a finite dimensional $\R$ vector space and $\det(V)\coloneq \Lambda^{\dim(V)}$ the maximal exterior Algebra (or Grassman Algebra).  by the principle of permanence we define $\Lambda^0(V)=\R$. An orientation of $V$ is a choice of one of the two connected component of $\dim(V)\setminus \{0\}$. 
\end{definition}
\begin{remark}
	Classically, an orientation of a vector space $V$ of dimension $m$ is given by an ordered basis $\mathfrak{b}=(b_1,\dots b_m)$ of $V$ and hence the produkt $b_1\wedge \dots \wedge b_m$ is an element of the maximal exterior product $\det(V)=\Lambda^mV$. Two ordered bases define different elements in $\det(V)$ and their difference is given by the determinant of the base change matrix. (This is just a calculation using multilinearity and the alternating property). Hence an choice of equivalence class of bases modulo basechange matrix of positive determinant is the same as a choice of one connected component of $\det(V)\setminus \{0\}$.
\end{remark}
...
induces a canonical isomorphism 
	...
	\end{align*} where $\tilde{c_j}\in g^{-1}(c_j)$ . The well definition of this map (invariance under choosen lift of $c$) can be calculated, since two different choices differ in an element in the kernal of $g$ which equals the image of $f$ and hence the different choices vanisch in the wedge produkt. By this isomorphism we get a orientation induced in the middle from the outside. Notice the convention, that the basis from $A$ is placed in front of the basis from $C$. 
	Notice how the definition is consistend with the edge cases of trivial vector spaces $A$ or $C$. If $A=0$ we get an isomorphism $B\cong C$ and a basis from $C$ induced to $B$, that is either the induced one from $g$, or exactly the opposite one, if the trivial vectorspace $A$ is negatively oriented.
\end{definition}
\begin{remark}
	Let 
	\begin{center}
...
be a short exact sequence of vector spaces and $\mathfrak{a}$ be a bases of $A$ , $\mathfrak{c}$ be a basis of $C$ and $\tilde{\mathfrak{c}}$ be a lift of $\mathfrak{c}$ via $g$. Replacing the basis $\mathfrak{a}$ with $\P\mathfrak{a}$ and $\mathfrak{c}$ with $Q\mathfrak{c}$ where $P,Q$ are invertible matricies,gives us a new induced basis on $B$, which differes from the original one by a martix $\begin{pmatrix}
	P&*\\
	0&Q
\end{pmatrix}$, where $*$ depends on the choosen lift. Hence the corresponding elements in $\det(B)$ differ by the product $\det(P)\det(Q)$. 
\end{remark}
\begin{lemma}[Orientation comparison in bundles]
...
\begin{remark}[Notiz für Mich]
	...
	The fiber above $p$ should be naturally isomophic to $T^u_pM$ by the direct sum decomposition and contractibility of $W(\to p)$ then gives us a trivialization and hence lets us induce an orientation over $T_xM\big/ T_xW(\to p)$. By transversality we get the short exact sequence of vector spaces that introduces a basis in the middle. 
	\begin{center}
		 \begin{tikzcd}
			0\arrow[r]&\arrow[r]T_x\gamma &\arrow[r]T_xW(q\to ) & T_xM\big/ T_xW(\to p) \arrow[r]&0
		 \end{tikzcd}
	\end{center}
	Anotherway to get the orientation in $T_xW(q\to)$ from $T^u_pM$ is by transporting the orthogonal complemenmt of $-\gamma_x(f)$ in $T_xW(q\to)$ towards $p$ along the curve $\gamma$. Here we have the problem, that we will never reach $T_pM$ as $p\notin \gamma$. To introduce an orientation that way we need to show that the tanged spaces converge into one another which is a corollary of the inclination or $\lambda$ lemma. 
\end{remark}

\begin{definition}
	Let $f:M\to \R$ be a morse function and $q,p$ two critical and assume that $\ind(q)-1=\ind(p)$. 
	Now take a point $x\in \gamma \in \mathcal{M}(q\to p)$ that lifes on a flow line between those points. 
...
induces a canonical isomorphism 
	\begin{align*}
		 \det(A)\otimes \det(C)& \to \det(B)\\
		 (a_1\wedge \dots\wedge a_l) \otimes (c_1\wedge \dots \wedge c_k) &\mapsto (f(a_1)\wedge \dots f(a_l)\wedge \tilde{c_1}\wedge \dots \wedge \tilde{c_k})\, ,
	\end{align*} where $\tilde{c_j}\in g^{-1}(c_j)$ . The well definition of this map (invariance under choosen lift of $c$) can be calculated, since two different choices differ in an element in the kernel of $g$ which equals the image of $f$ and hence the different choices vanish in the wedge product. By this isomorphism we get a orientation induced in the middle from the outside.
\end{definition}
\begin{remark}
	Let 
	\begin{center}
	...
	\end{tikzcd}
	\end{center}
	be an exact sequence of vector spaces and $\mathfrak{a}$ be a baseis of 
\end{remark}
...
Let $\gamma\in \mathcal{M}(q\to p)$. We extend $\gamma$ to also contain the $q$ and $p$ by compactifying the flow line. Now assume that $x\in \gamma\setminus \{q,p\}$. For this we define $N_{x}^{W(q\to)}{\gamma}$ be the normal bundle of $\gamma$ as a submanifold of $W(q\to)$. Since $\gamma$ is contractible we can trivialize said normal bundle to get an bundle isomorphism: 
\begin{equation*}
...
\end{equation*}
 The map $Q_p$ is the map from $\R^n\to T_p\R^n$ from definition \ref{def:vb-linearIdentification} and the map $E^u_q$ is an embedding from the refined unstablle manifold theorem.
...
\begin{proof}
	...
	\end{equation*} Since the flow line is transversal to the level set or in our case, the gradient is even orthogonal to the level sets, it is not contaied in $T_{\phi_{\tau(x)}(x)}f^{-1}(c)=  \ker\diff f \big|_{\phi_{\tau(x)}(x)}$ and hence:
	\begin{equation*}
...
\begin{proof} We denote the connected component of $f^{-1}(c)$ that containes $x_j$ by $V_j$. Now we choose a chart of $W(q\to)$ arround $x_j\in W(q\to)\cap V_j$ such that the induced bases of $T_{x_j}W(q\to)=\langle -\grad_{x_j}(f)\rangle \oplus T_{x_j}V_j$ respects that splitting by setting $\frac{\partial}{ \partial x^1}\big|_{x_j}= -\grad_{x_j}(f) $ and $\frac{\partial}{ \partial x^i}\big|_{x_j}\in T_{x_j}V_j$ for $i$ bigger than one. With this we can start our calculations:
	...
	\begin{equation*}
		\tau(\phi_s(x))=\tau(x)-s\,.
	\end{equation*} Differentiating this in the variable $s$ in the point $s'$ yealds:
	\begin{align*}
	...
	\end{equation*}
	We can furthermore define a basis of $T_x W(q\to)$ with $\mathfrak{b}_1\coloneq -\grad_x(f)$ and $\mathfrak{b}_i=\big(\restr{\diff \phi_{\tau(x)}}{x}\big)^{-1}({w_i})$ where $(w_i)$ denotes a basis of $T_{x_j}V_j$. Hence $w_i$ is orthogonal on $-\grad_{x_j}(f)$ for all $i>1$. Hence by the equation (\ref{eq: differential of tau}) we can caluclate:
	\begin{equation*}
	...
	\end{equation*}
	Finally after choosing a basis $(w_i)$, the map $x\mapsto \mathfrak{b}_i$ is smooth for all $i$ and any $x\in W(q\to)\setminus \{q\}$ and hence alltogether we get the desired frame.
\end{proof}
