...
\begin{proof}
	...
	\begin{equation*}
		H_i(D)=H^{-1}(C)\,,\quad \text{ and }\quad H_i(D')=H^{-1}(C')\,.
	\end{equation*} Furthermore, the redefinition is well behaved with tensor products in the sence that
	\begin{equation*}
		D\otimes_\Lambda D'=\bigoplus_{i+j=m}D_i\otimes D_j =\bigoplus_{i+j=m}D^{-i}\otimes D^{-j}=(C\otimes_{\Lambda}C')^{-m}\,
	\end{equation*}and the boundary map of the tensor product chain complex is the same as the coboundary map of the tensorproduct of the cochaincomplex (with the inverted degree).
	This leads to the equiality
	\begin{equation*}
		H_m(D\otimes_\Lambda D')=H^{-m}(C\otimes_\Lambda C')\,.
	\end{equation*}
	Now we can appliy the algebraic Künneth theorem to $D,D$ to get the short splitting exact sequence
	\begin{center}
		\begin{adjustbox}{width=\textwidth}
		\begin{tikzcd}
		0 \arrow[r]&
		\bigoplus_i\left(H_i(C)\otimes_\Lambda H_{m-i}(C')\right)\arrow[r]&
		H_m(C\otimes_\Lambda C') \arrow[r]&
		\bigoplus_i\left(\Tor_{\Lambda}\left(
		H_i(C),H_{m-i-1}(C')
		\right) \right) \arrow[r]&0
		\end{tikzcd}
