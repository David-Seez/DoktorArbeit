...
\end{proof}
\begin{definition}\label{def:morse-moduliSpaceOfFlowLines}
	For a Morse datum $(g,f,\mathfrak{o})$ and two critical points $p,q$ we define the spaces
	\begin{equation*}
		W(p\to q)\coloneq W(p\to) \cap W(\to q)\quad \text{and}\quad
		\mathcal{M}(p\to q)\coloneq W(p\to q)\big/ \R\,,
	\end{equation*} where the quotient is defined to be the set of orbits of the $\R$-action $t \mapsto \phi_t(x) $ for all $x\in W(p\to q)$. Hence, $\mathcal{M}(p\to q)$ is the set of flow lines and is called the \textbf{moduli space of flow lines}.
\end{definition}
...
\end{proof}
 ...
 \subsection{A Morse-Theoretik Künneth Formula}

\begin{definition}\label{def: Definition produce Morse datum}
	Let $(M,\mathfrak{A},\Lambda)$ and $(M',\mathfrak{A}',\Lambda)$ be two Morse data with $\mathfrak{A}=(g,f,\mathfrak{o})$ and $ \mathfrak{A}'=(g',f',\mathfrak{o}')$. On $M\times M'$ we define the Morse function $f+f'$ and the metric $g\oplus g'$. Then, $\diff_{(x,x')}(f+f')=\diff_x(f)+\diff_x'(f')$ and hence the critical points of $f+f'$ are pairs $(p,p')$ such that $p$ is critical for $f$ and $p'$ is critical for $f'$. By corollary \ref{cor:morse-localHessian} we conclude that the Hessian splits into block diagonal form and hence the property of non degenerayc follows. Since we use the product metric $g\oplus g'$ we also get a splitting of the gradient of $f+f'$: 
	\begin{align*}
		\diff_{(x,x')}(f+f')(V,V')
		&=\diff_x(f)(V)+\diff_{x'}(f')(V')\\
		&=g(\grad_x(f),V)+g'(\grad_{x'}(f'),V')\\
		&=(g\oplus g')(\underbrace{(\grad_x(f),\grad_{x'}(f'))}_{\grad_{(x,x')}(f+f')},(V,V'))\,.
	\end{align*} Hence, the flow also splits in the following sense: Assume $\tilde{\phi}$ is the flow of $f+f'$ on $M\times M'$, $\phi$ is the flow of $f$ on $M$ and $\phi'$ is the flow of $f'$ on $M'$. Then $\phi_t(x,x')=(\phi_t(x),\phi_t(x'))$, as the right sight satisfies the differential equation. Thereby, the (un-)stable manifolds split, letting us orient all unstable manifolds via that splitting which gives us $\mathfrak{o}\oplus \mathfrak{o}'$. Finally, transversal intersection is a property that carries over to products and hence the Morse datum $(M\times M',\mathfrak{A}\times \mathfrak{A}',\Lambda)$ with $\mathfrak{A}\times \mathfrak{A}'\coloneq (g\oplus g',f+f',\mathfrak{o}\oplus \mathfrak{o}')$ indeed constitutes a valid Morse datum.
	The splitting of the flow gives a splitting of the connecting flow lines and hence of the moduli spaces for two critical points $(q,q')$ and $(p,p')$: 
	\begin{equation*}
		\mathcal{M}\left((q,q')\to (p,p')\right)=\mathcal{M}(q\to p)\times \mathcal{M}(q'\to p') 
	\end{equation*} If $p\neq q$ and $p'\neq q'$, then for the moduli space to be non-empty we need 
	\begin{equation*}
		\ind(q)>\ind(p) \quad \text{and}\quad \ind(q')>\ind(p')\,.
	\end{equation*} This results in the index of $(q,q')$ being at least two bigger than the index of $(p,p')$. Hence, if we want to count flow lines between two consecutive indices, either $q$ equals $p$ or $q'$ equals $p'$. In this case we have the moduli space
	\begin{equation*}
		\mathcal{M}\left((q,q')\to (p,p')\right)=\begin{cases}
			p\times \mathcal{M}(q' \to p')&\text{ if } p=q\\
			\mathcal{M}(q \to p)\times p' &\text{ if } p'=q' \,.
		\end{cases}
	\end{equation*}
	Now, we want to analyse how the splitting affects the counting of flow lines. For this we denote the flow lines $\tilde{\gamma}\in \mathcal{M}\left((q,q')\to (p,p')\right)$, $\gamma \in \mathcal{M}{q\to p}$ and $\gamma'\in \mathcal{M}(q'\to p')$ such that $\tilde{\gamma}=\gamma\times \gamma'$.
	Then we get a short exact sequence of bundles:

	\begin{center}
		\begin{tikzcd}
			0 \arrow[r]& T\tilde{\gamma} \arrow[r]&\restr{TW\left((q,q')\to \right)}{\tilde{\gamma}}\arrow[r]&
			\restr{\Big(T(M\times M')\Big/ TW(\to (p,p')) \Big)}{\tilde{\gamma}} \arrow[r] &0
		\end{tikzcd}
	\end{center}
	...
	This sequence, however, is graded in the sense that all spaces (with the exception of the $0$) are direct sums and all maps respect them, meaning it is the direct sum of the two short exact sequences:
	\begin{equation}\label{eq:morse-firstExactSequence}
		\begin{tikzcd}
			0 \arrow[r]& T{\gamma} \text{ or }0\arrow[r]&\restr{TW\left(q\to \right)}{{\gamma}}\arrow[r]&
			\restr{\Big(TM\Big/ TW(\to p) \Big)}{{\gamma}} \arrow[r] &0 
		\end{tikzcd}
	\end{equation}
	and 
	\begin{equation}\label{eq:morse-secondExactSequence}
		\begin{tikzcd}
			0 \arrow[r]& T{\gamma'} \text{ or }0 \arrow[r]&\restr{TW\left(q'\to \right)}{{\gamma'}}\arrow[r]&
			\restr{\Big(TM'\Big/ TW(\to p') \Big)}{{\gamma'}} \arrow[r] &0
		\end{tikzcd}
	\end{equation}
	Now, we have the two orientations of $\restr{TW\left((q,q')\to \right)}{\tilde{\gamma}}$: The first one comes from the orientation of $T_{(q,q')}^uM\times M'$ and is given as the wedge product of two non-vanisching sections into the determinant bundle from the orientations of $T_q^uM$ and $T_{q'}^uM'$ and thereby represented by $s_q \otimes s_{q'}$( Remember that the determinant of a direct sum is the tensor product of the determinants; this is also true for bundles. Furthermore, we omit the needed inclusions or lifts to pair them with a $\wedge$.). 
	The other way of orienting comes from the exact sequences. Let $-\grad\wedge s_p$ denote a representative of the orientation induced by the short exact sequence (\ref{eq:morse-firstExactSequence}) and similarly denote the orientation induced in the middle of the second short exact sequence (\ref{eq:morse-secondExactSequence}) by $-\grad'\wedge s_{p'}$.  
	By definition, the comparison of $s_q$ with $-\grad\wedge s_p$ gives the sign $\epsilon(\gamma)$ and similarly the comparison of $s_{q'}$ with $-\grad'\wedge s_{p'}$ gives the sign $\epsilon(\gamma')$. For ease of notation we assume that the sections live in the right space. 

	Now we assume that $q'=p'$ resulting in the first element of the short exact sequence (\ref{eq:morse-secondExactSequence}) being trivial and hence giving us the need to compare those two sections:
	\begin{equation*}
		(-\grad )\wedge ( s_p\wedge s_{p'})\,.
	\end{equation*}

	Hence we need to compare the two orientations:
	\begin{equation*}
		s_q \wedge s_{q'} \quad \text{ with }\quad (-\grad )\wedge ( s_p\wedge s_{q'})=(-\grad\wedge s_p)\wedge s_{q'}\,.
	\end{equation*}
	It is now clear that the sign we get from comparing can be expressed as $\epsilon{(\gamma)}$.

	If $q=p$,  the first short exact sequence (\ref{eq:morse-firstExactSequence}) has a trivial first bundle and hence we need to compare the two orientations:

	\begin{equation*}
		s_q \wedge s_{q'} \quad \text{ with }\quad (-\grad' )\wedge ( s_q\wedge s_{q'})=(-1)^{\ind_q(f)}(s_q\wedge -\grad\wedge s_{p'})\,,
	\end{equation*}where the sign comes from moving $s_q$ to the front. Is is now clear that the sign we get from comparing can be expressed as $(-1)^{\ind_q(f)}\epsilon{(\gamma')}$.

This results for critical points $(q,q')$ and $(p,p')$ with consecutive indices in the formula:
	\begin{equation*}
		n\left((q,q'),(p,p') \right)=\begin{cases}
			(-1)^{\ind_q(f)}n(q',p') & \text{ if } q=p\\
			\phantom{(-1)^{\ind_q(f)}} n(q,p) & \text{ if } q'=p'			
		\end{cases}
	\end{equation*}
	\end{definition}
...
\begin{remark}
			Let $(M,\mathfrak{A},\Lambda)$ and $(M',\mathfrak{A}',\Lambda)$ be two Morse data with $\mathfrak{A}=(g,f,\mathfrak{o})$ and $ \mathfrak{A}'=(g',f',\mathfrak{o}')$. Then the map (given on the generators of the pure tensors)
		\begin{equation*}
			\Phi: \bigoplus_{i+j=k}C_i(M,\mathfrak{A},\Lambda)\otimes_\Lambda C_j(M',\mathfrak{A}',\Lambda) \to C_k(M\times M',\mathfrak{A}\times \mathfrak{A}',\Lambda)\,,\quad (p\otimes p')\mapsto (p,p')
		\end{equation*}
		is an isomorphism of groups.Furthermore, if we denote the boundary morphism of the chain complexes corresponding to $M,M'$ and $M\times M'$ by $\partial_f$, $\partial_{f'}$ and $\partial_{f+f'}$ respectifely, we	
		 obtain the following isomorphism of chain complexes induced by $\Phi$:

		\begin{multline*}
    \left(\bigoplus_{i+j=*}C_i(M,\mathfrak{A},\Lambda)\otimes_\Lambda C_j(M',\mathfrak{A}',\Lambda),\,\partial_f\otimes 1 +(-1)^i1\otimes \partial_{f'} \right)\\
    \to \left(C_*(M\times M',\mathfrak{A}\times \mathfrak{A}',\Lambda),\partial_{f+f'}\right)
\end{multline*}
		Here, the left side is,by definition, the tensor product of chain complexes. Hence, we get a natural isomorphism of chain complexes:
		\begin{equation*}
	...
	\begin{proof}
		The isomorphism of groups is easy to see, since we have a bijection of the generators
		\begin{equation*}
			(p\otimes q) \leftrightarrow (p,q)\,.
		\end{equation*} Since both sides are free $\Lambda$-modules we can apply the universal property of free modules, giving us a canonical isomorphism of $\Lambda$-modules and hence a group isomorphism.
		Now we want to show that we get an isomorphism of chain complexes and hence need to check if the boundary map commutes with $\Phi$. Let $q$ be a critical point of index $i$ of $f$ and let $q'$ be a critical point of index $j$ of $f'$. On the one hand, we can calculate
		\begin{align*}
    \Phi \left(\partial_f\otimes 1 +(-1)^i1\otimes  \partial_{f'}\right) &(q\otimes q')\\
    &= \Phi \left(\partial_f(q) \otimes q' +(-1)^i q\otimes  \partial_{f'}(q')\right)\\
    &= \Phi \left(\sum_{p\in \Crit_{i-1}(f)} n(q,p)\,p\otimes q' +(-1)^i q\otimes \sum_{p'\in \Crit_{j-1}(f')} n(q',p')\,p'\right) \\
    &=\sum_{p\in \Crit_{i-1}(f)} n(q,p)(p, q') +\sum_{p'\in \Crit_{j-1}(f')}(-1)^i n(q',p')(q,p')\,,
\end{align*} 

and on the other hand,
		\begin{align*}
    \partial_{f+f'}\left(\Phi(q \otimes q')\right)
    &=\partial_{f+f'}(q,q')\\
    &=\sum_{(p,p')\in \Crit_{i+j-1}(f+f')}n\left((q,q'),(p,p')\right)\,(p,p')\\
    &=\underbrace{\sum_{p\in \Crit_{i-1}(f)} n(q,p)(p,q')}_{q'=p'}+
    \underbrace{\sum_{p'\in \Crit_{j-1}(f')}(-1)^in(q',p')(q,p')}_{q=p}\,.
\end{align*}
	\end{proof}


\begin{lemma}[Algebraic Künneth Formula]\label{lem:morse-AlgKünForm}
	If $\Lambda$ is a principal ideal domain, and $C$ and $C'$ are chain complexes over $\Lambda$ such that all $C_n$ are free $\Lambda$-modules, then for each $n$ there is a natural short exact sequence 
	\begin{center}
...
\end{lemma}
\begin{proof}
	This is a standard result of homological algebra, found in most algebraic topology textbooks. Compare Theorem 3B.5. in  \cite{hatcher2002algebraic} for example.
\end{proof}


\begin{theorem}[Morse-theoretic Künneth Formula]
	Let $\Lambda$ be a principal ideal domain, and let $(M,\mathfrak{A},\Lambda)$ and $(M',\mathfrak{A}',\Lambda)$ be two Morse data. Then for each $n$ there is a splitting short exact sequence 
	\begin{multline*}
		0~\to ~
...
\end{theorem}
\begin{proof}
	The theorem follows directly from Lemma \ref{lem:morse-AlgKünForm} above together with the isomorphism of chain complexes 
	\begin{equation*}
		C_*(M,\mathfrak{A},\Lambda)\otimes_\Lambda C_*(M',\mathfrak{A}',\Lambda) \to C_*(M\times M',\mathfrak{A}\times \mathfrak{A}',\Lambda) \,.
	\end{equation*}
\end{proof}
\begin{remark}
	One might wonder, why there is no \textit{natural} short exact sequence in the Künneth Formula for Morse homology. This is due to the missing functoriality of Morse homology at the moment.
\end{remark}
...
\begin{equation*}
	...
	\end{cases*}
\end{equation*} 
are a basis of the dual group $C^k(M,\mathfrak{A},G)$. The coboundary map is easy to obtain too: Let $q_j$ for $j=1,\dots,r$ be the critical points with index $k+1$ and let $\xi_i$ be the corresponding forms. Let $n(q_j,p_i)$ be the number of flow lines form $q_j$ to $p_i$ counted with the sign. Then the coboundary map is given as:
\begin{align*}
	\partial^k:C^k(M,\mathfrak{A},G) & \to C^{k+1}(M,\mathfrak{A},G)\\
	\eta_i &\mapsto \partial^{k}\eta_i=\eta_i\circ \partial_{k+1}=\sum_j n(q_j,p_i) \xi_j \,.
\end{align*}
\end{definition}

\begin{corollary}[Algebraic Künneth Formula for Cohomology]
	If $\Lambda$ is a principal ideal domain, and $C$ and $C'$ are cochain complexes over $\Lambda$ such that all $C^n$ are free $\Lambda$-modules, then for each $n$ there is a natural short exact sequence 
	\begin{center}
		\begin{adjustbox}{width=\textwidth}
		\begin{tikzcd}
		0 \arrow[r]&
		\bigoplus_i\left(H^i(C)\otimes_\Lambda H^{n-i}(C')\right)\arrow[r]&
		H^n(C\otimes_\Lambda C') \arrow[r]&
		\bigoplus_i\left(\Tor_{\Lambda}\left(
		H^i(C),H^{n-i+1}(C')
		\right) \right) \arrow[r]&0
		\end{tikzcd}
...
\end{corollary}
\begin{proof}
	This is a corollary of the Algebraic Künneth Formula. For this we define the chain complexes \begin{equation*}
		D_{-i}\coloneq C^i\quad \text{ and }\quad {D'}_{-i}\coloneq {C'}^i
	\end{equation*}with the same differentials 
	\begin{equation*}
		d_i: D_i \to D_{i-1}\,,c\mapsto d^{-i}(c)  \quad \text{ and }\quad {d'}_i: {D'}_i\to {D'}_{i-1}\,,c'\mapsto d'^{-i}(c')
	\end{equation*}
	Then we have the equalities 
	\begin{equation*}
		H_i(D)=H^{-i}(C)\,,\quad \text{ and }\quad H_i(D')=H^{-i}(C')\,.
	\end{equation*} Furthermore, the redefinition is well behaved with tensor products in the sense that
	\begin{equation*}
		D\otimes_\Lambda D'=\bigoplus_{i+j=m}D_i\otimes D_j =\bigoplus_{i+j=m}C^{-i}\otimes C^{-j}=(C\otimes_{\Lambda}C')^{-m}\,
	\end{equation*}and the boundary map of the tensor product chain complex is the same as the coboundary map of the tensor product of the cochain complexes (with the inverted degree).
	This leads to the equality
	\begin{equation*}
		H_m(D\otimes_\Lambda D')=H^{-m}(C\otimes_\Lambda C')\,.
	\end{equation*}
	Now we can apply the Algebraic Künneth Formula to $D,D'$ to get the short splitting exact sequence
	\begin{center}
		\begin{adjustbox}{width=\textwidth}
		\begin{tikzcd}
		0 \arrow[r]&
		\bigoplus_i\left(H_i(D)\otimes_\Lambda H_{m-i}(D')\right)\arrow[r]&
		H_m(D\otimes_\Lambda D') \arrow[r]&
		\bigoplus_i\left(\Tor_{\Lambda}\left(
		H_i(D),H_{m-i-1}(D')
		\right) \right) \arrow[r]&0
		\end{tikzcd}
		\end{adjustbox}
	\end{center} We can now define $n\coloneq -m$ and reindex the summations from $i$ to $-i$, giving us the desired sequence.
\end{proof}

\begin{theorem}[Morse-theoretic Künneth Formula for Cohomology]
	Let $\Lambda$ be a principal ideal domain, and let $(M,\mathfrak{A},\Lambda)$ and $(M',\mathfrak{A}',\Lambda)$ be two Morse data. Then for each $n$ there is a splitting short exact sequence 
	\begin{multline*}
		0~\to ~
...
\begin{proof}
	...
	\end{equation*} This isomorphism ,however, is clear and given on the generators: Let $p$ and $p'$ be critical points of index $i$ and $j$ with respect to $f$ and $f'$. Let $\eta$ and $\eta'$ be the corresponding generators of $C^i(M,\mathfrak{A},\Lambda)$ and $C^j(M',\mathfrak{A}',\Lambda)$. Then we have the bijection 
	\begin{equation*}
		\eta \otimes \eta' \leftrightarrow (\eta,\eta')\,.
	\end{equation*} To get a chain isomorphism we need to check the compatibility with the coboundary maps. For each $q_l\in \Crit_{i+1}(f)$ let $\xi_l$ be the corresponding form and similarly for each ${q'}_k\in \Crit_{j+1}(f')$ let ${\xi'}_k$ be the corresponding form. Then we calculate
	\begin{align*}
	\Phi\left(
	 \partial^f\otimes 1+(-1)^i 1\otimes \partial^{f'}
	\right) (\eta \otimes \eta')
	&= \Phi\left(
	 \partial^f(\eta)\otimes \eta'+(-1)^i \eta\otimes \partial^{f'}(\eta')
	\right)\\
		&= \Phi\left(
	\sum_{l} n(q_l,p)\xi_l \otimes \eta'+\sum_k (-1)^i n({q'}_k,p')\eta\otimes {\xi'}_k
	\right)\\
		&= 
	\sum_{l} n(q_l,p)(\xi_l , \eta')+\sum_k (-1)^i n({q'}_k,p')(\eta, {\xi'}_k)
	\\
	\end{align*} and the other way around, if we define for $(q,q')$ with $\ind_{f+f'}(q,q')=\ind_{f+f'}(p,p')+1$ the corresponding form to be $\nu$ we have:
	\begin{align*}
		\partial^{f+f'}\Phi(\eta\otimes \eta')
		&=\partial^{f+f'}(\eta,\eta')\\
		&=\sum_{(q,q')\in \Crit_{i+j+1}(f+f')}n\left(
													(q,q'),(p,p')
											   \right)(\nu)\\
		&=\underbrace{\sum_l n(q_l,p)(\xi_l,\eta')}_{q'=p'}+
		  \underbrace{\sum_k (-1)^{i}n({q'}_k,p')(\eta,{\xi'}_k)}_{q=p}
	\end{align*}
	Now finally we can apply the Algebraic Künneth Formula for Cohomology and we are done. The latter formular is almost the same as the homological one but with a different index shift in the $\Tor$ functor. Again this is standard homological algebra.
\end{proof}
