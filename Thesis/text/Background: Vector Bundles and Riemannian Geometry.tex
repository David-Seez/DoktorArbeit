...
\begin{definition}\label{def: Vector Bundles and Riemannian Geometry-Frames from trivialisations}
...
\end{definition}
	...
	\begin{corollary}\label{cor: Vector Bundles and Riemannian Geometry- vector fields take functions}
...
\end{remark}
\begin{proof}
	We start by assuming that we have an atlas with structure group $\O(m)$. In Definition \ref{def: Vector Bundles and Riemannian Geometry-Frames from trivialisations} we saw that a covering of local trivializations is equivalent to a covering of smooth frames. Now in a single trivialization 
	\begin{equation*}
...
\begin{lemma}[Local Description of the Gradient]\label{lem: Vector Bundles and Riemannian Geometry-Local description of the Gradient}
...
\begin{proof} Assume that $v,w\in \Gamma( TU)$ such that $v=\sum_i v_i\partial_i$ and $w=\sum_i w_i\partial _i$ (Here $v_i$ and $w_i$ are smooth functions $U\to \R$). Then  $g(v,w)=\sum_{i,j} g_{ij} v_i w_j$.
	Suppose that $\grad(f)=\sum_j G^j \partial_j$ and $Q$ is the linear identification $T\R\to \R$ in each tangent space. Then by definition of the gradient and corollary \ref{cor: Vector Bundles and Riemannian Geometry- vector fields take functions} we have:
	\begin{align*}
...
\begin{remark}
	...
	\begin{equation*}
		t \mapsto \phi_t(x)\,,
	\end{equation*} for fixed $x$ at $t$ and evaluate this at the constant vector field $t\mapsto R_t(1)\in T_t \R$ from definition \ref{def: Vector Bundles and Riemannian Geometry- Linear identification}. 
\end{remark}
